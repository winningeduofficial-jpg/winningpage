...
function getBearerToken(req) {
  return String(req.headers.authorization || '').trim().replace(/^Bearer\s+/i, '');
}
...
export default async function handler(req, res) {
  ...
  if (!token) {
    return res.status(401).json({ ok: false, reason: 'not_authenticated', detail: '로그인이 필요합니다.' });
  }
  ...
  try {
    ...
    if (verificationError) {
      ...
      return res.status(500).json({ ok: false, reason: 'unknown', detail: '처리 중 오류가 발생했습니다.' });
    }
    ...
    if (updateError) {
      ...
      return res.status(500).json({ ok: false, reason: 'unknown', detail: '번호 변경에 실패했습니다.' });
    }
    ...
    if (consumeError) {
      ...
      console.error('[change-phone] phone_verifications 소비 마킹 실패:', consumeError);
    }

    return res.status(200).json({ ok: true, phone: rawPhone });
  } catch (error) {
    console.error('[change-phone] 오류:', error);
    return res.status(500).json({ ok: false, reason: 'unknown', detail: '번호 변경 중 오류가 발생했습니다.' });
  }
}
