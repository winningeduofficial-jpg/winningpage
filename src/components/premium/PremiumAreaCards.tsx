    ...
    >
      ...
      <ul className="mt-4 flex flex-col gap-2">
        {item.bullets.map((bullet) => (
          <li
            key={bullet}
            className={`flex items-start gap-2 break-keep text-[0.875rem] leading-[1.5] ${PREMIUM_NATURAL_TEXT_CLASS} group-hover:font-semibold group-hover:text-white`}
          >
            <span
              className="mt-[0.4rem] size-[0.3125rem] shrink-0 rounded-full bg-gold"
              aria-hidden="true"
