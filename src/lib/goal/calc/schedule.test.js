...
const SUM_CASES = [
  ...
  { name: 'undefined', input: undefined, expected: { weekIdeal: 0, weekMin: 0 } },
...
const EFFECTIVE_CASES = [
  ...
  { name: 'student null', args: [null, '2026-08-10', '2026-08-16'], expected: { ideal: 0, min: 0 } },
  {
  ...
  {
    name: '문자열 시간값',
    args: [{ study_schedule: { monday: { ideal: '6.5', min: '4.5' } } }, '2026-08-10', '2026-08-10'],
...
test('getEffectiveScheduleTarget — now 주입이 결과에 영향 없음 (구간이 명시된 경우)', () => {
  const a = getEffectiveScheduleTarget(STUDENT, '2026-08-10', '2026-08-16', new Date('2020-01-01T00:00:00Z'));
  const b = getEffectiveScheduleTarget(STUDENT, '2026-08-10', '2026-08-16', new Date('2030-12-31T00:00:00Z'));
...
const SUBSTRING_TRAP_CASES = [
  ...
  ['서울대학교', '자유전공학부(의예과 연계)', 0.9, '학과명에 의예과가 들어가면 의대 취급']
];
...
test('getStudyMultiplier — [parity] 카톨릭/가톨릭 오타 분기', () => {
  ...
  near(getStudyMultiplier('가톨릭대학교', '의예과'), 0.88, '정상 표기는 topMed 미포함');
});
...
const AVAIL_CASES = [
  ...
  { name: '학교·학원 없음', args: [day('7', '24', '', '', []), false], expected: 15.5 },
  {
    name: '주말 학원 2건',
    args: [
      day('7', '24', '', '', [
  ...
  { name: 'sleep === wake → 0', args: [day('7', '7', '', '', []), false], expected: 0 },
  { name: 'sleep < wake → 0', args: [day('20', '7', '', '', []), false], expected: 0 },
  { name: 'wake 빈문자(NaN) → 0', args: [day('', '24', '', '', []), false], expected: 0 },
  { name: 'sleep 빈문자(NaN) → 0', args: [day('7', '', '', '', []), false], expected: 0 },
  { name: '깨어있는 시간 1h → 음수라 0 clamp', args: [day('7', '8', '', '', []), false], expected: 0 },
  { name: '깨어있는 시간 정확히 1.5h → 0', args: [day('7', '8.5', '', '', []), false], expected: 0 },
  {
  ...
  {
    name: '소수 시각 7.5~23.5',
    args: [day('7.5', '23.5', '8.5', '16.5', [{ start: '17.5', end: '19.25' }]), true],
    expected: 4.8
  },
  {
    name: '숫자형 입력 (문자열 아님)',
  ...
  { name: '반올림 경계 0.05', args: [day('7', '8.55', '', '', []), false], expected: 0.1 }
];
...
test('calcAvailableHours — [parity] 기상(9시)이 등교(8시)보다 늦으면 가용시간이 더 깎인다', () => {
  ...
  near(calcAvailableHours(day('9', '24', '8', '16', []), true), 4.5, 'available');
  // 기상 8시(등교와 동일)면 가산항이 0이라 (24-8) -1.5 -8 +0 = 6.5
  near(calcAvailableHours(day('8', '24', '8', '16', []), true), 6.5, 'available');
});
...
test('calcAvailableHours — [parity] day 또는 academies 누락이면 TypeError', () => {
  ...
  assert.throws(
    () => calcAvailableHours({ wake: '7', sleep: '24', schoolStart: '', schoolEnd: '' }, false),
    TypeError
  );
});
...
const WEEK_CASES = [
  ...
  },
  {
    ...
    form: {
      ...
      minUniv: '부산대학교',
      minDept: '경영학과',
      weekSchedule: weekOf(day('7', '7', '', '', []), day('7', '7', '', '', [])),
...
test('calculateWeekSchedule — [parity] 병리② 목표가 가용시간을 초과 (주말 가용 15.5h · 자습 15h → ideal 16h)', () => {
  ...
  assert.ok(got.saturday.ideal > weekendAvail, '이상 목표가 가용시간을 초과해야 한다(병리 유지)');
...
const APPROX_CASES = [
  {
    ...
    args: [{ wakeUpHour: 7, sleepHour: 24, schoolStayHours: 8, academyHours: 2 }, true],
    expected: 7
  },
  {
    name: '온보딩 기본값 — 주말(학교체류 미차감)',
    args: [{ wakeUpHour: 7, sleepHour: 24, schoolStayHours: 8, academyHours: 2 }, false],
    expected: 15
  },
  {
    name: '학교·학원 0',
    args: [{ wakeUpHour: 7, sleepHour: 24, schoolStayHours: 0, academyHours: 0 }, true],
    expected: 17
  },
  {
    name: '과다 차감 → 0 clamp',
    args: [{ wakeUpHour: 7, sleepHour: 12, schoolStayHours: 8, academyHours: 4 }, true],
    expected: 0
  },
  {
    name: '취침 === 기상 → 0',
    args: [{ wakeUpHour: 7, sleepHour: 7, schoolStayHours: 0, academyHours: 0 }, true],
    expected: 0
  },
  {
    name: '취침 < 기상 → 0',
    args: [{ wakeUpHour: 23, sleepHour: 7, schoolStayHours: 0, academyHours: 0 }, true],
    expected: 0
  },
  ...
  { name: '빈 객체 → 0', args: [{}, true], expected: 0 },
  {
    ...
    args: [
      { wakeUpHour: '7', sleepHour: '23.5', schoolStayHours: '7.5', academyHours: '1.25' },
      true
    ],
    expected: 7.8
  },
  ...
  {
    name: '학교체류 NaN → 항 무시',
    args: [{ wakeUpHour: 7, sleepHour: 24, schoolStayHours: '', academyHours: 2 }, true],
    expected: 15
  },
  {
    name: '상한 초과 학교체류 24 → 0 clamp',
    args: [{ wakeUpHour: 7, sleepHour: 24, schoolStayHours: 24, academyHours: 0 }, true],
    expected: 0
  },
  {
    name: 'hasSchool 기본값은 true',
    args: [{ wakeUpHour: 7, sleepHour: 24, schoolStayHours: 8, academyHours: 2 }],
    expected: 7
  }
];
...
const ERROR_DIRECTION_CASES = [
  {
    ...
    steppers: { wakeUpHour: 7, sleepHour: 24, schoolStayHours: 8, academyHours: 2 },
    hasSchool: true,
    academyCount: 1,
    preSchoolHours: 1,
    orig: 5.5,
  ...
  },
  {
    ...
    day: day('7', '24', '8', '16', []),
    steppers: { wakeUpHour: 7, sleepHour: 24, schoolStayHours: 8, academyHours: 0 },
    hasSchool: true,
    academyCount: 0,
    preSchoolHours: 1,
    orig: 8.5,
  ...
  },
  {
    ...
    steppers: { wakeUpHour: 7, sleepHour: 24, schoolStayHours: 8, academyHours: 3 },
    hasSchool: true,
    academyCount: 2,
    preSchoolHours: 1,
    orig: 3.5,
  ...
  },
  {
    ...
    day: day('5', '23', '8', '16', []),
    steppers: { wakeUpHour: 5, sleepHour: 23, schoolStayHours: 8, academyHours: 0 },
    hasSchool: true,
    academyCount: 0,
    preSchoolHours: 3,
    orig: 11.5,
  ...
  },
  {
    ...
    day: day('8', '24', '8', '16', []),
    steppers: { wakeUpHour: 8, sleepHour: 24, schoolStayHours: 8, academyHours: 0 },
    hasSchool: true,
    academyCount: 0,
    preSchoolHours: 0,
    orig: 6.5,
  ...
  },
  {
    ...
    steppers: { wakeUpHour: 7, sleepHour: 24, schoolStayHours: 0, academyHours: 2 },
    hasSchool: false,
    academyCount: 1,
    preSchoolHours: 0,
    orig: 12.5,
  ...
  },
  {
    ...
    day: day('7', '24', '', '', []),
    steppers: { wakeUpHour: 7, sleepHour: 24, schoolStayHours: 0, academyHours: 0 },
    hasSchool: false,
    academyCount: 0,
    preSchoolHours: 0,
    orig: 15.5,
