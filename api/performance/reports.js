...
function toDesignReport(row) {
  if (!row) return null;
  const envelope = row.sections && typeof row.sections === 'object' ? row.sections : {};
  return {
    reportId: row.id,
...
async function handleList(req, res, supabaseAdmin, userId) {
  ...
  const limit = Number.isInteger(rawLimit) ? Math.min(Math.max(rawLimit, 1), LIST_LIMIT_MAX) : LIST_LIMIT_DEFAULT;
  ...
  if (viewError) throw new Error(`저장 리포트 목록 조회 실패: ${viewError.message}`);
...
async function handleDetail(req, res, supabaseAdmin, userId, sessionId) {
  ...
  if (reportsError) throw new Error(`리포트 조회 실패: ${reportsError.message}`);

  const rows = reportRows || [];
  const designRow = rows.find((row) => row.report_type === 'design') || null;
  const evaluationRow = rows.find((row) => row.report_type === 'evaluation') || null;
  const finalRow = rows.find((row) => row.report_type === 'final_submission') || null;
  ...
  if (submissionsError) throw new Error(`제출본 조회 실패: ${submissionsError.message}`);
