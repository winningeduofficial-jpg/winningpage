 ...
 * COPY_FALLBACK 에는 URGENT_GOAL_REACHED('목표 수준 도달')·VALUE_MISSING('미입력') 두 개뿐이고
 * 둘 다 특정 슬롯 전용 문장이라, 문구 조회 실패 자리에 되돌리면
 *   (a) 진단 서술 카드에 '미입력'이 렌더되고,
 *   (b) 호출부의 `?? 폴백` 사슬이 조용히 끊긴다(문자열은 truthy 다).
 * 범용 "문구 없음" 문장을 새로 만드는 것도 불가 — §5.3 정적 검사 대상에 창작 문구가 섞이고
 * CASE-10 문구 개수 검산(341+22)이 어긋난다. 따라서 슬롯별 폴백 결정은 전부 buildReport 가 소유하고,
