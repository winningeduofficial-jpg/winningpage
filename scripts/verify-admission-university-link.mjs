...
const META_MODAL_REL = 'src/components/admission/editor/AdmissionMetaEditModal.jsx';
...
function sliceExactlyOnce(source, start, end, what) {
  ...
  if (endIdx === -1) {
    throw new Error(`${what} 슬라이스 종료 앵커를 찾지 못했다: ${JSON.stringify(end)}`);
  }
...
async function loadResourceRow() {
  ...
  const externalUrlFn = sliceExactlyOnce(src, 'function externalUrl(value) {', '\nfunction ', 'externalUrl');
...
function selectItems(sqlText, whatFile) {
  ...
  if (!m) throw new Error(`${whatFile}: create or replace view … select … from 구간을 파싱하지 못했다.`);
  const items = [];
  let depth = 0;
  let cur = '';
  for (const ch of m[1]) {
...
async function main() {
  ...
  const renderRow = (row) =>
    renderToStaticMarkup(React.createElement(RowHarness, { university, row, onOpenInfo: () => {} }));

  // 이름 셀의 <a> 만 센다(스텁 버튼은 <span> 이라 섞이지 않는다).
  const nameAnchors = (html) => html.match(/<a\b[^>]*admission-directory-name-link[^>]*>/g) || [];
  ...
  // ── link:2 — target/rel 보안 속성 ───────────────────────────────────
  {
    ...
    const pass =
      tag.includes('target="_blank"') && rel.includes('noopener') && rel.includes('noreferrer');
    record(
      'link:2',
  ...
  // ── link:3 — URL 없음 → 평문 ────────────────────────────────────────
  {
    ...
    const pass = nameAnchors(html).length === 0 && html.includes('가톨릭관동대학교');
    record(
      'link:3',
      'official_source_url 이 없으면 <a> 를 만들지 않고 대학명을 평문으로 렌더한다(빈 링크 금지)',
      pass,
      JSON.stringify({ anchors: nameAnchors(html), hasName: html.includes('가톨릭관동대학교') })
    );
  }
  ...
  // ── link:4 — row=null(미매칭 대학) ──────────────────────────────────
  {
    ...
    const pass = threw === null && nameAnchors(html).length === 0 && html.includes('가톨릭관동대학교');
    record(
      'link:4',
  ...
  {
    const catHtml = renderRow({ detail_status: 'category', official_source_url: 'https://kps.ac.kr' });
    const normalHtml = renderRow({ detail_status: 'normal', official_source_url: 'https://kps.ac.kr' });
  ...
  // ── link:7 — 죽은 href 부재 ─────────────────────────────────────────
  {
    ...
    const pass = !combined.includes('href="#"') && !combined.includes('href=""');
    record(
      'link:7',
      '어떤 입력에서도 href="#" / href="" 같은 죽은 링크를 만들지 않는다',
      pass,
      JSON.stringify({ hash: combined.includes('href="#"'), empty: combined.includes('href=""') })
    );
  }
    ...
    });
  {
    const html = renderToStaticMarkup(
      renderCell({ key: 'university_name', type: 'universityNameMeta' }, () => {})
    );
  ...
  // ── admin:10 — 클릭하면 onOpenMetaEdit(row) 가 정확히 1회 ───────────
  {
    ...
    const button = findNode(element.type(element.props), (n) => n.type === 'button');
    ...
    record(
      ...
      pass,
      JSON.stringify({ hasButton: Boolean(button), metaClicks: metaClicks.length, editClicks: editClicks.length })
    );
  }
  ...
  {
    const WRAP = (inner) => `<table><tbody><tr><td class="px-3 py-3">${inner}</td></tr></tbody></table>`;
    ...
    const noHandler = renderToStaticMarkup(
      renderCell({ key: 'university_name', type: 'universityNameMeta' }, undefined)
    );
  ...
  // 이 기능의 최대 위험(뷰에 컬럼이 없으면 조용히 전멸)을 기계로 막는다.
  {
    ...
    const matches = fs.readdirSync(sqlDir).filter((f) => f.endsWith(VIEW_SQL_GLOB_SUFFIX));
    let pass = false;
    let detail;
    if (matches.length !== 1) {
      detail = JSON.stringify({ matches, expected: `sql/<번호>${VIEW_SQL_GLOB_SUFFIX} 1개` });
    } else {
      ...
      const hasGrant = /grant\s+select\s+on\s+public\.admission_university_resource_index\s+to\s+anon,\s*authenticated/i.test(
        newSql
      );
      pass =
        prefixOk &&
        ...
        hasGrant;
      detail = JSON.stringify({ file: matches[0], prefixOk, appended, hasGrant, baseCount: baseItems.length });
    }
    record(
      'sql:12',
