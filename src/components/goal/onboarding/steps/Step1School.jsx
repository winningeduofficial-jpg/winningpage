...
import { HIGH_SCHOOL_GRADE_OPTIONS, SCHOOL_TYPE_OPTIONS } from '../../../../data/goalOnboardingMock';
...
export default function Step1School({ goNext }) {
  ...
  return (
    <>
      ...
      >
        <div role="radiogroup" aria-label="학교 유형" className="flex flex-wrap gap-[0.75rem]">
      ...
      {isHighSchoolPath && (
        <QuestionCard step="1-2" label="학교정보" title="현재 학년을 선택해 주세요.">
          <div role="radiogroup" aria-label="학년" className="flex flex-wrap gap-[0.75rem]">
      ...
      {isBlockedPath && (
        <QuestionCard step="1-2" label="학교정보" title="아직 준비 중이에요">
          <p className="text-[0.875rem] leading-[1.6] text-ink-sub">
            중학교・초등학교는 학년 수와 성적 입력 방식이 달라 아직 지원하지 않아요. 일반고 또는
            특목・자사고를 선택하면 계속 진행할 수 있어요.
          </p>
        </QuestionCard>
      )}
