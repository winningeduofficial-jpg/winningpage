...
export default function SliderRow({ label, value, onChange, min = 0, max = 12, step = 1 }) {
  ...
  return (
    <div className="flex items-center gap-[1.25rem]">
      <span className="w-[2.75rem] shrink-0 text-[0.875rem] text-ink">{label}</span>
      ...
      <input
        ...
        style={{ background: `linear-gradient(to right, #0B84FD ${pct}%, #D9D9D9 ${pct}%)` }}
      />
      ...
      <span className="w-[2.25rem] shrink-0 text-center text-[0.875rem] font-medium text-ink">{value}h</span>
      <button
        type="button"
        onClick={increase}
