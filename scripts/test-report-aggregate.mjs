...
test('computeStudyHours — study_hours 합', () => {
  const hours = computeStudyHours([{ study_hours: 3.5 }, { study_hours: '2.0' }, { study_hours: null }]);
...
test('computeAchievementRate — 요일별 목표 합 대비 % (일요일 제외)', () => {
  ...
  const effectiveWindow = { start: '2026-08-03', end: '2026-08-04', elapsedDays: 2 }; // 월·화만
...
test('computeAchievementRate — effectiveWindow 없으면 0%', () => {
  const result = computeAchievementRate({ totalStudyHours: 10, student: { study_schedule: {} }, effectiveWindow: { start: null, end: null, elapsedDays: 0 } });
  assert.deepEqual(result, { idealRate: 0, minRate: 0, idealTargetHours: 0, minTargetHours: 0 });
});
...
test('computeCompletionScore — 과제 0건이면 0.5/0.3 → 0.625/0.375 재정규화', () => {
  ...
  const score = computeCompletionScore({ idealRate: 100, recordDays: 3, elapsedDays: 3, doneTasks: 0, totalTasks: 0 });
  assert.equal(score, 100);
});

test('computeCompletionScore — 과제 있으면 0.5/0.3/0.2 3축 가중', () => {
  // achievement=50, consistency=50, plan=50 → 0.5*50+0.3*50+0.2*50 = 50
  const score = computeCompletionScore({ idealRate: 50, recordDays: 1, elapsedDays: 2, doneTasks: 1, totalTasks: 2 });
  assert.equal(score, 50);
});

test('computeCompletionScore — idealRate가 100 넘어도 achievementAxis는 100으로 clamp', () => {
  const score = computeCompletionScore({ idealRate: 150, recordDays: 2, elapsedDays: 2, doneTasks: 2, totalTasks: 2 });
...
test('computeCohortPercentile — 코호트 2명 미만이면 표본 부족', () => {
  const result = computeCohortPercentile('me', [{ profileId: 'me', hours: 10 }]);
...
test('computeConditionBreakdown — 기록이 없는 컨디션도 0일로 노출', () => {
  const { listRows, tiles } = computeConditionBreakdown([{ body_condition: 'great', study_hours: 5 }]);
...
test('bucketTimeSlots — 자정 직후(KST 01시) 세션은 심야 0~6 버킷', () => {
  const buckets = bucketTimeSlots([{ started_at: '2026-08-03T16:30:00.000Z', duration_seconds: 3600 }]); // UTC 16:30 = KST 01:30
...
test('bucketTimeSlots — KST 21시는 오후 9~12 버킷, 경계(24시)는 넘치지 않는다', () => {
  const buckets = bucketTimeSlots([{ started_at: '2026-08-03T12:00:00.000Z', duration_seconds: 1800 }]); // UTC 12:00 = KST 21:00
  const late = buckets.find((b) => b.label === '오후 9~12');
  assert.equal(late.value, 0.5);
  assert.equal(buckets.reduce((s, b) => s + b.value, 0), 0.5);
});
...
test('computeDistraction — "없었음"은 차트에서 제외', () => {
  const rows = computeDistraction([{ reasons: ['스마트폰', '없었음'] }, { reasons: ['스마트폰'] }]);
...
test('computeCoreItems — 내림차순 정렬', () => {
  const rows = computeCoreItems([{ tasks: ['오답 정리'] }, { tasks: ['오답 정리', '개념 학습'] }]);
...
test('computeAdmissionDelta — 상승/하락 방향과 절대값', () => {
  const targets = { ideal: { university: '서울대학교', department: '경영학과' }, min: { university: '건국대학교', department: '경영학과' } };
  const result = computeAdmissionDelta({
    targets,
    startLog: { ideal_susi: 20, ideal_jungsi: 15, min_susi: 30, min_jungsi: 25 },
    endLog: { ideal_susi: 22.5, ideal_jungsi: 14, min_susi: 30, min_jungsi: 25 }
  });
...
console.log('report-aggregate 테스트 정의 완료 — node:test 러너가 위 test() 블록을 실행합니다.');
