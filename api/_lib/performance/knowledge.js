...
// 알려진 부채다.
function resolveFilterSubject({ includeOtherSubjects, subject }) {
 ...
 *
 * @param {object} params
 * @param {import('@supabase/supabase-js').SupabaseClient} [params.supabase] 미지정 시
 *   `createSupabaseAdmin()`. RPC는 SECURITY INVOKER이고 테이블 RLS가 `is_admin()`이라
 ...
 *
 * @returns {Promise<Array<object>>} RPC 행 배열. 검색 실패는 빈 배열이다 — 과거 수행은
