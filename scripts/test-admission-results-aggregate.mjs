...
check('같은 전형명이라도 중심전형이 다르면 표 행이 분리된다 (983그룹 회귀)', () => {
  const rows = [
    row({ main_track: '교과', admission_track: '학교장추천전형', grade_50: 2.1 }),
    row({ main_track: '종합', admission_track: '학교장추천전형', grade_50: 3.4 })
  ];
  ...
  const active = tableRows.map((r) => r.cells.find((c) => c.year === ACTIVE_RESULT_YEAR).value);
...
check('같은 (중심전형, 전형명)의 2개년 행은 한 행으로 묶여 연도 셀 2개가 된다', () => {
  const rows = [
    row({ result_year: 2025, main_track: '교과', admission_track: '일반전형', grade_50: 2.5 }),
    row({ result_year: 2026, main_track: '교과', admission_track: '일반전형', grade_50: 2.2 })
  ];
  const tableRows = buildTableRows(rows);
...
check('중심전형이 null인 행들은 하나의 null 그룹으로만 묶인다', () => {
  const rows = [
    ...
    row({ main_track: '', admission_track: '일반전형', grade_50: 3.0, result_year: 2025 })
  ];
  const tableRows = buildTableRows(rows);
  assertEqual(tableRows.length, 1, '표 행 수');
...
check('특수교육은 기회균형이 아니라 특수교육으로 분류된다 (706행 흡수 회귀)', () => {
  // screening_category 경로
  assertEqual(categorize(row({ screening_category: '특수교육' })).key, 'special', '컬럼값 경로');
  // 정규식 fallback 경로 — 과거 opportunity 규칙이 삼키던 자리
  assertEqual(
    categorize(row({ screening_category: null, admission_track: '특수교육대상자전형' })).key,
    'special',
    '정규식 fallback 경로'
  );
});
...
check('정규식 fallback이 11종 각각을 올바른 탭으로 보낸다', () => {
  ...
  cases.forEach(([track, expected]) => {
    const actual = categorize(row({ screening_category: null, admission_track: track })).key;
...
check('논술·실기는 main_track 축으로도 잡힌다', () => {
  assertEqual(
    categorize(row({ screening_category: null, main_track: '논술', admission_track: '논술우수자' }))
      .key,
    'nonsul',
    '논술'
  );
  assertEqual(
    categorize(row({ screening_category: null, main_track: '실기', admission_track: '실기우수자' }))
      .key,
    'practical',
    '실기'
  );
...
check('screening_category 컬럼값 11종이 모두 자기 탭으로 매핑된다', () => {
  ...
  Object.entries(columnValues).forEach(([value, expected]) => {
    ...
    const actual = categorize(row({ screening_category: value, admission_track: '일반전형' })).key;
  ...
  CATEGORY_ORDER.forEach((key) => {
    if (!covered.has(key)) throw new Error(`CATEGORY_ORDER의 "${key}"를 덮는 케이스가 없다`);
  });
});

check('탭은 행이 있는 카테고리만 CATEGORY_ORDER 순서로 생성된다', () => {
  const rows = [
    row({ screening_category: '지역인재', admission_track: '지역인재전형', grade_50: 3.0 }),
    row({ screening_category: '일반', admission_track: '일반전형', grade_50: 2.0 }),
    row({ screening_category: '논술', main_track: '논술', admission_track: '논술전형', grade_50: 4.0 })
  ];
...
check('Δ 비교 불가(incomparable) — 전년 없음은 "신규", 올해 없음은 "-"', () => {
  ...
  const onlyPrevious = computeDelta(cell(2025, 2.4, 50), cell(2026, null, null));
...
check('Δ 컷 기준 상이(cut_mismatch) — 값·방향은 살고 톤만 낮아진다', () => {
  ...
  assertEqual(d.direction, DELTA_STATE.IMPROVED, 'direction은 살아 있어야 한다');
...
check('pickGrade는 50%컷을 최우선으로 쓴다', () => {
  assertDeepEqual(
    pickGrade(row({ grade_50: 2.1, grade_70: 2.5, grade_85: 3.0, grade_90: 3.4 })),
    { value: 2.1, cut: 50 },
    '50컷 우선'
  );
});
...
check('pickGrade는 50·70이 둘 다 없고 85만 있으면 85를 쓴다 (§8.4 확장 59행)', () => {
...
check('pickGrade는 90%컷까지 내려가고, 전부 없으면 null', () => {
  assertDeepEqual(pickGrade(row({ grade_90: 5.1 })), { value: 5.1, cut: 90 }, '90컷');
  assertEqual(formatGradeCell(row({ grade_90: 5.1 })), '5.10 (90)', '90컷 표기');
...
check('pickGrade는 문자열 numeric과 빈 문자열을 방어한다', () => {
  assertDeepEqual(pickGrade(row({ grade_50: '2.10' })), { value: 2.1, cut: 50 }, '문자열 numeric');
  assertDeepEqual(
...
const GENERAL_ONLY_ROWS = [
  row({ result_year: 2026, screening_category: '일반', admission_track: '일반전형', quota: 10, grade_50: 2.0 }),
  row({ result_year: 2025, screening_category: '일반', admission_track: '일반전형', quota: 10, grade_50: 2.2 })
];
...
check('일반 행이 있으면 다른 유형이 섞여 있어도 일반만으로 평균한다', () => {
  const mixed = [
    ...
    row({ result_year: 2026, screening_category: '지역인재', admission_track: '지역인재전형', quota: 40, grade_50: 4.0 })
  ];
  ...
  assertEqual(mixedCard.value, pureCard.value, '혼합 입력이 일반-only와 같아야 한다');
  assertEqual(mixedCard.rowCount, 3, 'rowCount는 중심전형 전체 행 수');
  assertEqual(mixedCard.basisRowCount, 2, 'basisRowCount는 기준 행 수');
});

check('일반 행이 없으면 모집인원 가중 최다 유형을 기준으로 삼고 basis를 알린다', () => {
  const rows = [
    row({ screening_category: '농어촌', admission_track: '농어촌전형', quota: 5, grade_50: 3.0 }),
    row({ screening_category: '지역인재', admission_track: '지역인재전형', quota: 40, grade_50: 4.0 })
  ];
...
check('요약 카드 라벨은 표본 연도 수에 따라 "가중평균" 단어를 붙이거나 뺀다', () => {
...
check('분할모집(같은 키 2행)은 모집인원 합계 · 경쟁률 평균으로 집계된다', () => {
  const rows = [
    row({ result_year: 2026, quota: 10, competition_rate: 8.0, grade_50: 2.0 }),
    row({ result_year: 2026, quota: 15, competition_rate: 10.0, grade_50: 3.0 })
  ];
  const [tableRow] = buildTableRows(rows);
  ...
  assertEqual(tableRow.activeCompetitionRateDisplay, '9.00 : 1', '경쟁률 표기');
...
check('연도 셀 3상태 — 값 / 미공개 / 행 부재', () => {
  ...
  const [onlyActive] = buildTableRows([row({ result_year: 2026, grade_50: 2.4 })]);
...
check('경쟁률 0 행은 평균 계산의 분모에서도 빠진다', () => {
  ...
  assertEqual(allZero[0].activeCompetitionRateDisplay, EMPTY_CELL, '전부 0이면 "-"');
...
check('집계 모델 전체에 "0.00 : 1" 문자열이 한 곳도 없다', () => {
  ...
  walk(model);
  if (leaks.length > 0) throw new Error(`경쟁률 0이 렌더 경로로 샜다: ${leaks.join(', ')}`);
});
...
check('축(2025·2026) 밖 연도 행은 모델 진입부에서 전량 걸러진다', () => {
  ...
  assertEqual(model.trackSummaries[0].value, 2.0, '축 밖 행이 평균에 섞이지 않는다');
});
