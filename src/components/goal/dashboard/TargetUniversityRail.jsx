...
function RateRow({ label, value, dotClassName, fillClassName, available = true }) {
  return (
    <div className="flex items-center gap-2">
      {/* line(#d7d7d7)은 이 코드베이스의 "비활성" 토큰이다(tailwind.config.js 주석: 비활성
          버튼·보더·미체크) — 별도 disabled 톤을 새로 만들지 않고 기존 관례를 따른다. */}
      <span aria-hidden="true" className={`h-2 w-2 shrink-0 rounded-full ${available ? dotClassName : 'bg-line'}`} />
...
function UniversityCard({ label, university, department, susiRate, jeongsiRate, jungsiAvailable }) {
  return (
