...
async function loadHarness() {
  ...
  const harnessPath = path.join(
    REPO_ROOT,
    `.tmp-perf-sidebar-harness-${stamp}.jsx`,
  );
  const bundlePath = path.join(
    REPO_ROOT,
    `.tmp-perf-sidebar-bundle-${stamp}.mjs`,
  );
...
for (const { pathname, expected } of CASES) {
  ...
  check(
    html.includes('aria-labelledby="perf-nav-heading"') &&
      html.includes('id="perf-nav-heading"'),
...
console.log(
  `✓ 수행평가 사이드바 메뉴 활성 표시 검증 통과 (경로 ${CASES.length}종)`,
);
