...
//
// 공개 대학모집요강 모달 "껍데기" 골든 검증 — scripts/verify-admission-modal-shell.mjs
// 의 검증(읽기 전용) 절반을 이식. 골든 재생성 CLI(--capture)는 테스트가
// 아니라 사람이 리팩터 착수 전 수동으로 한 번 돌리는 도구라 별도 독립
// 스크립트 scripts/capture-admission-modal-shell-golden.mjs로 분리했다(이
// 파일에서 옮기지 않는다 — package.json test/verify/CI 어디에도 연결 안 함).
//
// 왜 필요한가
// -----------
...
//      골든을 비교하는 CLI 전용 모드였다 — Node/jsdom 렌더와 비교하는 자동
//      스텝이 원래도 없었다) 골든 파일 자체가 온전한지만 확인한다.
//
//  (B) SSR 바이트 골든 — scripts/__fixtures__/admission-modal-shell.ssr.json
//      AdmissionGuidelines.tsx 의 모달 JSX 영역을 **소스에서 기계적으로
//      잘라내** 하네스로 감싸고 renderToStaticMarkup 한 결과. 이 파일이
//      매 실행마다 검증하는 대상 — 슬라이스는 **현재 소스 파일**에서 다시
//      뜨므로 사본이 아니라 실제 페이지 파일을 검사한다.
//
// 원본 fixture JSON 2개는 절대 수정/재생성하지 않는다 — 골든은 리팩터
// 이전 상태의 정답이다.
//
// 이식 메모(node:test → Vitest, task 10.5)
// -----------------------------------------
...
//   esbuild에도 물려 vite.config.js의 별칭과 동일하게 맞췄다(과거 AdmissionGuidelines.jsx는
//   전부 상대 경로 import라 이 문제가 없었지만, TS 전환 후 "@/" 절대경로로
//   바뀌었다 — task 8).
// - .jsx→.tsx 경로 드리프트: PAGE_REL을 AdmissionGuidelines.tsx로 수정.
...
//   헤더 주석 — "공개 화면 DOM 바이트 불변을 지키는 규칙", 원문 그대로 옮긴
//   의도적 리팩터). 그 결과 원래 슬라이스 앵커 **안**에서 직접 계산되던
...
//   동일하게 재현한다(AdmissionModalShell의 "ref → data-section → className
//   순서" 불변 규칙 덕에 바이트 출력은 여전히 골든과 같아야 한다).
// - jsdom 환경 보정: 이 골든은 "DOMParser가 없는 순수 Node SSR" 조건에서
//   떴다(당시 저장소에 jsdom 자체가 없었다 — SafeHtml.tsx의
//   `typeof DOMParser === "undefined"` 분기가 항상 참이라 html 케이스는 항상
//   null/래퍼만 남는 축약 경로를 탔다). vitest.config.ts의 기본 환경은
//   task 10.1에서 도입한 jsdom인데, 그대로 두면 두 가지가 깨진다 — (1)
//   SafeHtml이 전역 DOMParser를 실제로 발견해 html 케이스에서 파싱을
//   시도하므로 이 게이트가 잡으려는 모달 껍데기 회귀와 무관한 바이트
//   diff가 나고, (2) jsdom이 패치한 전역(TextEncoder 등)이 esbuild의 자체
//   무결성 검사("new TextEncoder().encode("") instanceof Uint8Array")와
//   충돌해 esbuild.build() 자체가 예외를 던진다(둘 다 실측 확인).
//   그래서 이 파일 맨 위에 `@vitest-environment node` 지시어로 이 파일만
//   순수 Node 환경에서 돌린다 — 골든이 전제한 조건이 자연히 재현되고(전역
//   DOMParser 자체가 없음), esbuild도 문제없이 동작한다. html 콘텐츠
//   자체의 실제 sanitize 검증은 SafeHtml.test.tsx/AdmissionSectionView.test.tsx가
//   맡는다 — 이 파일의 관심사는 모달 껍데기 바이트뿐이다.
//
...
async function loadHarness(harnessSource: string) {
  ...
  try {
    const result = await esbuild.build({
      ...
      // AdmissionGuidelines.tsx가 "@/" 절대경로 import를 쓴다(task 8).
      alias: { "@": path.join(REPO_ROOT, "src") },
      external: ["react", "react-dom", "react/jsx-runtime", "react-dom/server"],
      write: false,
    });
    const outputFile = result.outputFiles[0];
    if (!outputFile) throw new Error("esbuild가 출력 파일을 만들지 않았다.");
...
> = {
  ...
  // 이 골든이 뜬 조건(전역 DOMParser 없음)에서는 SafeHtml이 null 을 반환한다
  // (SafeHtml.tsx `typeof DOMParser === 'undefined'` 분기). 즉 이 케이스는
  // 본문 내용이 아니라 **html 분기의 래퍼 마크업**을 고정한다. 실제 html
  // 본문 렌더는 SafeHtml.test.tsx / AdmissionSectionView.test.tsx가 덮는다.
  html: {
    selectedInfo: {
      ...BASE,
      status: "ready",
...
async function renderAllCases(sourceText: string) {
  ...
  // `@vitest-environment node` 지시어 덕에 이 파일엔 전역 DOMParser가 아예
  // 없다 — 골든이 전제한 조건이 자연히 재현된다(상단 헤더 주석 "jsdom 환경
  // 보정" 절 참고).
...
describe("SSR 바이트 골든 (scripts/__fixtures__/admission-modal-shell.ssr.json)", () => {
  test("케이스 집합이 골든과 일치", () => {
  ...
  test.each(Object.keys(CASES))("ssr:%s 바이트 완전 일치", (name) => {
    const golden = JSON.parse(fs.readFileSync(SSR_FIXTURE, "utf8"));
