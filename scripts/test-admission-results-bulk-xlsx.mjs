...
check(
  "정상 왕복: export한 행을 다시 parse하면 update로 분류되고 값이 보존된다",
  () => {
    ...
    assert(
      ...
      `잘린 셀이 있음(${truncatedCells.length}건) — 선행 조건 실패`,
    );

    const roundTripped = roundTripWorkbook(workbook);
    const existingIdSet = new Set([original.id]);
    const { rows, errors, warnings, summary } =
      parseAdmissionResultRowsFromXlsx(roundTripped, existingIdSet);

    assert(
      errors.length === 0,
      `에러 ${errors.length}건: ${JSON.stringify(errors)}`,
    );
    ...
    assertEqual(
      ...
      original.competition_rate,
      "competition_rate",
    );
    assertEqual(row.waitlist_rank, original.waitlist_rank, "waitlist_rank");
    assertEqual(
      row.subject_reflection,
      original.subject_reflection,
      "subject_reflection",
    );
...
check(
  "정상 왕복: 등급·경쟁률 등 수치 컬럼은 export 시점에 숫자 타입을 유지한다(문자열로 안 바뀜)",
  () => {
...
check("id가 빈 셀이면 insert로 분류되고 payload에 id 키가 없다", () => {
  ...
  assert(
    !("id" in rows[0]),
    "insert 행 payload에 id 키가 있으면 안 됨(DB가 자동 채번)",
  );
});
...
check(
  "파일 안에서 8축 자연키가 중복되면 두 번째 행이 duplicateNaturalKey로 거부된다",
  () => {
...
check(
  "main_track이 다르면(교과 vs 종합) 나머지 7축이 같아도 중복이 아니다",
  () => {
    ...
    assertEqual(
      errors.length,
      0,
      `에러가 없어야 함: ${JSON.stringify(errors)}`,
    );
    assertEqual(rows.length, 2, "두 행 모두 남아야 함");
  },
);
...
check("result_year가 2025/2026이 아니면 invalidResultYear로 거부된다", () => {
  ...
  assert(
    RESULT_YEAR_OPTIONS.includes(2026),
    "선행 조건: RESULT_YEAR_OPTIONS에 2026 포함",
  );
});
...
check("main_track이 CHECK 도메인 밖이면 invalidMainTrack으로 거부된다", () => {
  ...
  assert(
    MAIN_TRACK_OPTIONS.includes("교과"),
    "선행 조건: MAIN_TRACK_OPTIONS에 교과 포함",
  );
});
...
check(
  "screening_category가 CHECK 도메인 밖이면 invalidScreeningCategory로 거부된다",
  () => {
    ...
    assert(
      SCREENING_CATEGORY_OPTIONS.includes("일반"),
      "선행 조건: SCREENING_CATEGORY_OPTIONS에 일반 포함",
    );
  },
);

check(
  "필수값(university_key 등)이 비어 있으면 missingRequiredFields로 거부된다",
  () => {
    ...
    assert(
      errors[0].reason.includes("university_key"),
      "에러 사유에 university_key가 명시돼야 함",
    );
  },
);

check(
  "등급이 numeric(4,2) 범위(절댓값 100 미만)를 벗어나면 invalidGrade로 거부된다",
  () => {
...
check(
  "잘림 마커가 있는 셀이 있으면 truncatedColumn으로 행 전체가 거부된다",
  () => {
...
check(
  "경고: 등급 9종이 전부 비어 있으면 allGradesEmpty 경고가 남고 행은 정상 반영된다",
  () => {
    const wb = buildWorkbook([
      dbRow({
        id: 1,
...
check(
  "경고: 경쟁률이 0이면 competitionRateZero 경고가 남고 값은 0 그대로 반영된다(자동 null 변환 없음)",
  () => {
    const wb = buildWorkbook([dbRow({ id: 1, competition_rate: 0 })]);
    const { rows, warnings } = parseAdmissionResultRowsFromXlsx(
      wb,
      new Set([1]),
    );
    assertEqual(
      rows[0].competition_rate,
      0,
      "경쟁률 0이 그대로 저장돼야 함(임의 null 변환 금지)",
    );
...
check(
  "경고: 50%컷이 70%컷보다 낮은 등급(숫자가 큼)이면 gradeCutInversion 경고가 남는다",
  () => {
...
check(
  "summary: insert/update/skip/경고 건수가 실제 rows/errors/warnings와 일치한다",
  () => {
    const rowsInput = [
      {
        ...dbRow({
    ...
    const { rows, errors, warnings, summary } =
      parseAdmissionResultRowsFromXlsx(wb, new Set([1, 2]));
    ...
    assertEqual(
      rows.length,
      summary.willInsert + summary.willUpdate,
      "rows.length === insert+update",
    );
    ...
    assertEqual(summary.errorCounts.idNotFound, 1, "errorCounts.idNotFound");
    assertEqual(
      summary.warningCounts.competitionRateZero,
      1,
      "warningCounts.competitionRateZero",
    );
  },
);

check(
  "sheetNotFound: 빈 워크북(시트 없음)을 넘기면 에러 1건만 나오고 죽지 않는다",
  () => {
    ...
    assertEqual(
      summary.willSkip,
      0,
      "시트 자체가 없으면 willSkip은 0(행 단위 집계가 아님)",
    );
  },
);
