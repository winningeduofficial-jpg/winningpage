...
if (!fs.existsSync(SOURCE_HTML)) {
  console.log(
    `SKIP verify-performance-submission-schema — 이식 원본 없음 (${SOURCE_DIR})`,
  );
  process.exit(0);
}
...
if (sliceStart < 0 || sliceEnd < 0 || sliceEnd <= sliceStart) {
  console.error(
    "FAIL: 원본에서 제출 스키마 구간을 찾지 못했습니다 (index.html 구조 변경?).",
  );
  process.exit(1);
}
...
console.log("\n[우선순위 전수 대조]");
check(
  `트리거 조합 ${1 << TRIGGERS.length}가지 전건 일치`,
  comboMismatch === 0,
  comboFirst,
);
check(
...
for (const qCase of QUESTION_CASES) {
  ...
  if (qCase.expectFields !== undefined) {
    check(
      ...
      actual.type === "question_based" &&
        actual.fields.length === qCase.expectFields,
...
check(
  `후보 필드 조합 ${1 << LENGTH_PARTS.length}가지 전건 일치 (관측 형태 ${lengthShapes.size}종)`,
  lengthMismatch === 0,
  lengthFirst,
);
...
for (const text of LENGTH_DIRECT) {
  ...
  if (JSON.stringify(expected) !== JSON.stringify(actual)) {
    ...
    console.log(
      `  FAIL ${JSON.stringify(text)} — 원본 ${JSON.stringify(expected)} / 이식 ${JSON.stringify(actual)}`,
    );
  }
}
check(
  `inferLengthBasedSchema 단독 ${LENGTH_DIRECT.length}건 일치 (null 폴백 포함)`,
  lengthDirectMismatch === 0,
);
...
check(
  `정규화 ${NORMALIZE_CASES.length}건 일치 (rows 제외)`,
  normalizeMismatch === 0,
);
...
check(
  ...
  !/function\s+isRubricLikeQuestion/.test(portText) &&
    /from '\.\/guide-structure\.js'/.test(portText),
);
...
check(
  ...
  ported.countFieldChars("가  나   다") === 5 &&
    ported.countFieldChars("가 나 다") === 5,
);
check(
  "코드 포인트 단위 (이모지 1자)",
...
check(
  ...
  ported.countSubmissionChars(basic, { intro: "가", junk: "나".repeat(500) })
    .total === 1,
);
check(
  "char_counts는 스키마 전 필드 키를 갖는다 (빈 값도 0으로)",
  JSON.stringify(
    Object.keys(ported.countSubmissionChars(basic, { intro: "가" }).perField),
...
check(
  "임계값 정확히 100자 → 통과",
  pass.total === 100 && pass.ok === true && pass.threshold === 100,
);
...
check(
  ...
  fail99.total === 99 &&
    fail99.ok === false &&
    fail99.missingRequired.length === 0,
);
...
check(
  ...
  JSON.stringify(dupOriginal.fields.map((f) => f.key)) ===
    JSON.stringify(["question_1", "question_1"]),
);
check(
...
console.log(
  "PASS verify-performance-submission-schema — 제출 스키마 8종이 원본과 동일합니다.",
);
