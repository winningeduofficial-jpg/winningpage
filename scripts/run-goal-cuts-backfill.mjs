...
import { fetchBackfillSourceRows, computeGoalCutBackfill } from '../src/lib/goal/goalCutBackfill.js';
...
async function main() {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    throw new Error('.env.local 에서 SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY 를 찾을 수 없습니다.');
  }
  ...
  console.log(`[1/4] admission_results 소스 행 읽는 중(yearMode=${YEAR_MODE})...`);
  ...
  for (let i = 0; i < chunks.length; i += 1) {
    const { error } = await supabase.from(GOAL_CUTS_TABLE).upsert(chunks[i], { onConflict: GOAL_CUTS_CONFLICT });
    if (error) {
      errorCount += 1;
    ...
    okChunks += 1;
    console.log(`  청크 ${i + 1}/${chunks.length} 성공 (${chunks[i].length}행)`);
  }
