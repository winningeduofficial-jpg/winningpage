...
function renderSanitized(sanitizeToReact, html, className) {
  ...
  return renderToStaticMarkup(React.createElement(Tag, { className }, result.children));
}
...
async function main() {
  ...
  const render = (html, className) => renderSanitized(sanitizeToReact, html, className);
  ...
  // 8) STRIP_SUBTREE_TAGS 확장 — RAWTEXT/RCDATA 콘텐츠 스푸핑 방어
  {
    ...
    for (const [label, input] of cases) {
      const out = render(input);
      const ok = out.includes('앞') && out.includes('뒤') && !out.includes('<b>') && !out.includes('bold') && !out.includes('img') && !out.includes('onerror');
      allPass = allPass && ok;
      detail.push(`${label}: ${ok ? 'OK' : 'FAIL'} → ${out}`);
    }
    record('8. STRIP_SUBTREE_TAGS 확장(title/textarea/noembed/noframes/xmp/plaintext/template/noscript) 자식까지 제거', allPass, detail.join('\n'));
  }
  ...
  // 9) 크기 상한 — html.length > MAX_HTML_LENGTH(512KB)면 평문(<pre>)으로 격하
  {
    ...
    const pass = Boolean(result) && result.degraded === true && typeof result.children === 'string' && result.children.length > 500000;
    record('9. 크기 상한(512KB) 초과 시 평문 격하', pass, `degraded=${result?.degraded} length=${result?.children?.length}`);
  }
  ...
  // 10) 노드 수 상한 — 태그 5만 개 초과면(문자열은 512KB 미만) 평문으로 격하
  {
    ...
    record('10. 노드 수 상한(50,000) 초과 시 평문 격하(크기는 상한 밑)', pass, `input.length=${manyTags.length} degraded=${result?.degraded}`);
  }
  ...
  //     반환해 STRIP_SUBTREE_TAGS 검사를 거치지 않은 script 내용이 샜다.
  {
    ...
    record('11. 깊이 상한(100) 초과 서브트리 안의 script 내용 유출 방지', pass, out);
  }

  // 12) ATTR_TO_PROP 프로토타입 체인 오염 방어
  {
    const input = '<table><tbody><tr><td constructor="x" __proto__="y">z</td></tr></tbody></table>';
    const out = render(input);
    const pass = out.includes('>z<') && !out.includes('constructor') && !out.includes('__proto__') && !out.includes('[object');
    record('12. ATTR_TO_PROP constructor/__proto__ 속성 무시(프로토타입 체인 미조회)', pass, out);
  }
  ...
  //     globalThis.DOMParser 셔임으로만 노드에서 검증 가능하다(prop 없음).
  {
    ...
    let threw = false;
    try {
      out = renderToStaticMarkup(React.createElement(SafeHtml, { html: '<div>x</div>', className: 'admission-existing-html' }));
    } catch (err) {
      threw = true;
      out = String(err);
    } finally {
    ...
    const pass = !threw && out.includes('class="admission-existing-html"') && out.includes('>x<');
    record('13. SafeHtml 컴포넌트(prop 없이, globalThis.DOMParser 셔임) 정상 동작', pass, out);
  }
