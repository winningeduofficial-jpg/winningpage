...
export default function GoalAchievementCard({ title, variant = 'weekly', rows, weeks }) {
  const ROW_FILL = ['bg-accent', 'bg-[#ABDFBA]'];

  return (
    <GoalCard tone="neutral" className="flex min-h-[12.5rem] flex-col gap-5 px-6 py-6">
      ...
      {variant === 'monthly' && weeks?.length > 0 && (
        <>
          ...
          <div className="flex flex-1 flex-col justify-center gap-3">
            {weeks.map((week) => (
              <div key={week.label} className="flex flex-wrap items-center gap-3 text-[0.75rem] leading-[1.3]">
                <span className="w-10 shrink-0 font-semibold text-ink-strong">{week.label}</span>
                <div className="flex min-w-[8rem] flex-1 items-center gap-2">
                  <span className="w-8 shrink-0 text-ink-sub">최소</span>
                  <GoalProgressBar value={week.min} max={100} thickness="0.375rem" className="flex-1" />
                  <span className="w-9 shrink-0 text-right font-medium text-ink-strong">{week.min}%</span>
                </div>
                <div className="flex min-w-[8rem] flex-1 items-center gap-2">
                  ...
                  <GoalProgressBar
                    ...
                    className="flex-1"
                  />
                  <span className="w-9 shrink-0 text-right font-medium text-ink-strong">{week.upper}%</span>
                </div>
              </div>
            ))}
          </div>
