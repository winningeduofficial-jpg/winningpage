...
// 공개 페이지 탭(전체/자사고/외고/영재고/과학고) 및 DB CHECK 제약과 동일하게 유지할 것.
...
const CONFIGS = {
  ...
  specialHighschool: {
    ...
    order: 'sort_order',
    homepage: true,
    guideText: `특목고 합격 페이지의 합격 사례 카드입니다. 구분(자사고/외고/영재고/과학고)이 공개 페이지의 탭 필터와 그대로 연결되며, 여기 없는 구분은 선택할 수 없습니다. 학생명은 반드시 마스킹해 입력하세요(허용 문자: O, ○, *, 예: 홍O동) — 실명 노출은 개인정보 위반입니다. 출신 중학교는 있는 경우에만 입력하며, 비워 두면 카드에 표시되지 않습니다. 순서 숫자가 작을수록 앞에 나옵니다. 상단 합격률 숫자는 '연도별 합격률' 탭에서 관리하고, 그 아래 대학 로고 줄은 '수시정시합격 > 대학 로고'에서 관리합니다(두 페이지가 같은 로고를 공유합니다).`,
    columns: [
