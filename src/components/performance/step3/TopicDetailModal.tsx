...
// 직접 import해 `DialogPrimitive.Popup`을 조립했다(`DialogContent`의 기본 클래스가 이 모달의
// 실측 치수·배경색과 충돌한다는 판단). 지금은 `DialogContent`에 `className`(폭·라운드·배경 전부
// 여기서 덮어쓴다, `cn()`이 tailwind-merge로 충돌을 해소한다)과 `overlayClassName`(딤 색)을 넘겨
// 같은 결과를 낸다 — Base UI 프리미티브를 이 파일에서 다시 조립하지 않는다.
//
// **닫기 수단이 하나로 수렴한다.** 시안 단정: 모달에 X 버튼이 없고 닫기 수단은
// `다른 주제 보기` 버튼뿐이다. 그러나 §10.2 P9가 ESC/딤 클릭/포커스 트랩을 명시적으로
// 요구하므로 셋 다 지원하되(Base UI Dialog 내장), 전부 `다른 주제 보기`와 같은 동작(모달을
// 닫고 카드 목록으로 복귀)으로 수렴시킨다 — `onOpenChange`가 `false`를 받으면 `onClose`
// 하나만 부른다.
//
...
//
// **접근성**: `role="dialog"`/포커스 트랩/포털은 Base UI Dialog가 담당한다(`DialogContent`가
// 내부에서 `Portal`+`Backdrop`+`Popup`을 조립) — 예전에는 이 컴포넌트가 앱 루트에 대한
// 참조가 없는 저수준 모달이라 형제 트리에 `inert`를 직접 걸 수 없었지만, Base UI Portal +
// Backdrop 조합으로 그 제약이 해소됐다. 다만 `aria-modal`은 Base UI Popup이 자동 배선하지
// 않으므로 `DialogContent`에 `aria-modal="true"`를 리터럴로 넘겨 보강한다(`DialogContent`는
// 알지 못하는 props를 그대로 Popup에 전달한다).
...
export default function TopicDetailModal({
  ...
  return (
    ...
    >
      {/* 딤 — `#00000066`(검정 40%, §5.11 실측 = `performance-dim` 토큰). 블러는 유지한다
          (`ReportModalShell`과 달리 이 모달의 딤 실측은 무블러 요구가 없다). X 버튼은
          시안에 없으므로 `showCloseButton={false}`. */}
      <DialogContent
        showCloseButton={false}
        // Base UI Popup은 aria-modal을 자동 배선하지 않는다 — 리터럴로 명시.
        // `DialogContent`는 알지 못하는 props를 그대로 내부 Popup에 전달한다.
        aria-modal="true"
        overlayClassName="bg-performance-dim"
        ...
        // 디자인 확인 시 조정될 수 있다.
        // `sm:max-w-190.5`는 `DialogContent` 기본값의 `sm:max-w-sm`을 지우기 위한 중복
        // 지정이다(tailwind-merge는 변형 접두사가 다르면 별개 슬롯으로 봐서 접두사 없는
        // 값만 줘서는 `sm:` 규칙이 안 지워진다).
        className="fixed top-1/2 left-1/2 z-100 flex h-203.75 max-h-[90vh] w-[calc(100%-2rem)] max-w-190.5 sm:max-w-190.5 -translate-x-1/2 -translate-y-1/2 flex-col gap-0 overflow-hidden rounded-2xl border border-performance-line bg-[#fbfbfa] p-0 text-base text-ink ring-0 shadow-[0_24px_60px_rgba(0,0,0,0.24)] outline-none"
      >
        ...
        {/* 본문 — 폭 41.875rem이 정본(§5.11), 좌 인셋 2.125rem 고정. 키보드 스크롤이
            ...
            Base UI Dialog의 초기 포커스가 패널 내 첫 포커서블 엘리먼트를 잡으므로, 라벨이
            없으면 모달이 열리는 순간의 첫 낭독이 무음이다.
            ...
            값으로 둔다(검토 B-2). */}
        <section
          // biome-ignore lint/a11y/noNoninteractiveTabindex: APG Scrollable Regions 패턴 — Base UI Dialog의 초기 포커스가 이 요소를 잡는다.
          tabIndex={0}
          aria-label="주제 상세 내용"
        ...
        <DialogFooter className="mx-0 mb-0 flex h-20 shrink-0 flex-row items-center justify-start gap-5 sm:justify-start rounded-b-2xl border-t border-performance-line bg-white p-0 px-8.5">
          <button
            type="button"
            onClick={onClose}
