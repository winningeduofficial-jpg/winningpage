...
export function PopoverStatus({ title, description, onRetry, tone = 'default' }) {
  const titleClass = tone === 'error' ? 'text-red-600' : 'text-[#525252]';

  return (
    <div className="px-6 py-10 text-center">
      <p className={`break-keep text-base font-semibold ${titleClass}`}>{title}</p>
      {description ? (
        <p className="mt-2 break-keep text-sm font-medium text-[#8f8f8f]">{description}</p>
      ) : null}
      {onRetry ? (
        <button
          type="button"
