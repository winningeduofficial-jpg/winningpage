...
import { fetchGoalGrades, addGoalGrade } from '../../../lib/goalApi';
    ...
    <GoalCard tone="neutral" className="flex h-full flex-col gap-5 px-[2rem] py-[1.75rem]">
      ...
      {history.length > 0 ? (
        <div className="mt-auto flex flex-col gap-2">
          <p className="text-[0.9375rem] font-bold leading-[1.4] text-ink-strong">기록한 성적</p>
          <div className="flex flex-col gap-2">
            {history.map((record) => (
              <div key={record.term} className="flex items-center justify-between text-[0.875rem] text-ink">
      ...
      <AddMockExamGradeModal open={modalOpen} onClose={() => setModalOpen(false)} onSubmit={handleSubmit} />
    </GoalCard>
  );
}
