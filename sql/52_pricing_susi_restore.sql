...
--     ('52_pricing_susi_restore_v1') 로 최초 1회만. 30/32/50번에 선례가 있다.
...
--
-- ⚠️ 확인 못 한 것: anon 키로는 products RLS("products public read" =
--    is_active = true)를 넘을 수 없어서, susi-* 가 "행 자체가 없음"인지
--    "is_active = false 로 내려간 상태"인지 구분하지 못했다.
--    그래서 두 경우를 두 문장으로 나눠 각각 1회성으로 처리한다.
...
--
-- service_sort_order = 2 : dev 의 현재 서비스 순서가 goal 1 / (빈칸 2) /
--   mentor 3 / suhaeng 4 / diagnose 99 라 2번 슬롯이 그대로 비어 있다.
