...
import { CONDITION_MULTIPLIER, calculateDailyBonusV2 } from '../src/lib/goal/calc/bonusV2.js';
...
test('달성률 170% 초과 · great · 태그 2종 모두 → 1.2 × 1.1 × 1.1 = 1.452배', () => {
  ...
  assert.equal(getAchievementRateMultiplier(200), 1.2, '사전 조건: 200% 는 1.2배 구간');
  ...
  assertClose(r.idealSusiBonus, round4(RATES.idealSusiRate * multiplier), 'idealSusiBonus');
  assertClose(r.idealJungsiBonus, round4(RATES.idealJungsiRate * multiplier), 'idealJungsiBonus');
  assertClose(r.minSusiBonus, round4(RATES.minSusiRate * multiplier), 'minSusiBonus');
  assertClose(r.minJungsiBonus, round4(RATES.minJungsiRate * multiplier), 'minJungsiBonus');
});
...
test('목표 시간 0 이하 → 그 목표만 달성률 100% 간주(분모 0 회피)', () => {
  ...
  assertClose(r.idealSusiBonus, RATES.idealSusiRate, 'idealSusiBonus (100% 간주)');
  assertClose(r.idealJungsiBonus, RATES.idealJungsiRate, 'idealJungsiBonus (100% 간주)');
  assertClose(r.minSusiBonus, round4(RATES.minSusiRate * 0.3), 'minSusiBonus (30% 실배수)');
  assertClose(r.minJungsiBonus, round4(RATES.minJungsiRate * 0.3), 'minJungsiBonus (30% 실배수)');
});

test('과목 태그는 수시/정시로 갈린다 — 내신 과목은 수시만, 기출/모의고사는 정시만', () => {
  const base = { ...RATES, condition: 'normal', studyHours: 10, idealHours: 10, minHours: 10 };
  ...
  assertClose(plain.idealSusiBonus, RATES.idealSusiRate, 'plain idealSusiBonus');
  assertClose(plain.idealJungsiBonus, RATES.idealJungsiRate, 'plain idealJungsiBonus');

  assertClose(naesin.idealSusiBonus, round4(RATES.idealSusiRate * 1.1), '내신 → 이상수시 1.1배');
  assertClose(naesin.minSusiBonus, round4(RATES.minSusiRate * 1.1), '내신 → 최소수시 1.1배');
  assertClose(naesin.idealJungsiBonus, RATES.idealJungsiRate, '내신은 정시 미영향');
  assertClose(naesin.minJungsiBonus, RATES.minJungsiRate, '내신은 최소정시 미영향');

  assertClose(mock.idealJungsiBonus, round4(RATES.idealJungsiRate * 1.1), '기출 → 이상정시 1.1배');
  assertClose(mock.minJungsiBonus, round4(RATES.minJungsiRate * 1.1), '기출 → 최소정시 1.1배');
...
test('컨디션 미지값·빈 문자열은 normal(1.0)로 취급된다(카드-only 제출 허용)', () => {
  const base = { ...RATES, tasks: [], studyHours: 10, idealHours: 10, minHours: 10 };

  const withEmpty = calculateDailyBonusV2({ ...base, condition: '' });
  const withUndefined = calculateDailyBonusV2({ ...base, condition: undefined });
...
test('0시간 감점 분기가 없다 — v1과 달리 studyHours=0도 정상 수식을 그대로 태운다', () => {
  ...
  assert.equal(getAchievementRateMultiplier(0), 0, '사전 조건: 0% 달성률 배수는 0');
