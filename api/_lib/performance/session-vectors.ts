...
type ReportSection = { id?: string; label?: string; blocks?: ReportBlock[] };

export function flattenReportSectionsToText(
  sections: ReportSection[] = [],
): string {
  if (!Array.isArray(sections) || sections.length === 0) return "";

  function flattenBlock(block: ReportBlock): string[] {
 ...
 */
type SessionVectorSearchTextInput = {
  gradeLabel?: string;
  subjectGroup?: string;
  subject?: string;
  careerGoal?: string;
...
export function buildSessionVectorSearchText({
  ...
  topicTitle,
  summaryText,
}: SessionVectorSearchTextInput = {}): string {
  return [
    `학년: ${gradeLabel || ""}`,
    `교과군: ${subjectGroup || ""}`,
    `과목: ${subject || ""}`,
 ...
 */
export function computeContentHash(text: unknown): string {
