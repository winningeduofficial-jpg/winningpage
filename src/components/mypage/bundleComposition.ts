...
export type BundleGrantSpecRow = {
  product_id: string;
  program_key: string;
  duration_months: number | null;
  session_quota: number | null;
...
// product_id → 구성 권한 원값(program_key/duration_months/session_quota) 배열.
// MyServicesTab이 번들 주문을 서비스별 가상 카드로 펼칠 때 쓴다(마이페이지
// "나의 서비스" QA 지적, 2026-09-01) — useBundleCompositionMap은 사람이 읽는
// 라벨 문자열만 주지만, 카드 분리는 order_name 파서(MyServicesTab.tsx의
// DURATION_BRACKET_RE·MONTHS_RE·COUNT_RE)가 그대로 먹을 수 있는 원값(개월수·
// 회차)이 따로 필요해 별도 훅으로 둔다.
export function useBundleGrantSpecMap(
  productIds: (string | null | undefined)[] | undefined,
) {
  const uniqueIds = Array.from(
    new Set((productIds ?? []).filter((id): id is string => Boolean(id))),
  ...
  const [map, setMap] = useState<Map<string, BundleGrantSpecRow[]>>(new Map());
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        .from("bundle_items")
        .select("product_id, program_key, duration_months, session_quota")
        .in("product_id", key.split(","))
        .returns<BundleGrantSpecRow[]>();

      if (!alive) return;

      // 조회 실패 시 번들 전개를 조용히 생략한다(useBundleCompositionMap과
      // 같은 방침) — 이 경우 호출부(expandOrder)는 단일 카드 폴백으로
      // 자연히 넘어간다(번들 아님과 동일 경로).
      if (error || !data) {
        setMap(new Map());
        return;
      }
      ...
      const next = new Map<string, BundleGrantSpecRow[]>();
      for (const row of sorted) {
        const list = next.get(row.product_id) ?? [];
        list.push(row);
        next.set(row.product_id, list);
