...
check("왕복 — DB 행을 내보냈다가 그대로 읽으면 payload 가 보존된다", () => {
  ...
  assert(
    ...
    `거부가 없어야 한다: ${JSON.stringify(errorTypes(result))}`,
  );
  assert(
  ...
  assert(
    a.id === 11 && a.cut_type === "normal" && a.avg_cut === 2.35,
    "normal 행이 보존되지 않았다",
  );
  assert(
  ...
  assert(
    b.id === 12 && b.cut_type === "jungsi" && b.avg_cut === 87.5,
    "jungsi 행이 보존되지 않았다",
  );
  assert(
    b.is_active === false && b.note === "운영 메모",
    "jungsi 의 노출/메모가 보존되지 않았다",
  );
});

check(
  "왕복 — key 2컬럼은 엑셀에 없고 파서가 표시명에서 복사한다(§3-D5)",
  () => {
    ...
    assert(
      !grid[0].includes("university_key"),
      "university_key 가 엑셀 컬럼으로 나갔다",
    );
    assert(
      !grid[0].includes("department_key"),
      "department_key 가 엑셀 컬럼으로 나갔다",
    );
    ...
    assert(
      payload.university_key === "연세대",
      "university_key 가 표시명에서 복사되지 않았다",
    );
    assert(
      payload.department_key === "경영학과",
      "department_key 가 표시명에서 복사되지 않았다",
    );
  },
);
...
check("왕복 — 컬럼 순서를 뒤섞어도 헤더 이름으로 매핑된다", () => {
  ...
  assert(
    result.errors.length === 0,
    `거부가 없어야 한다: ${JSON.stringify(result.errors)}`,
  );
  const [payload] = result.rows;
  assert(
    payload.cut_type === "special" && payload.avg_cut === 3.1,
    "뒤섞인 컬럼이 잘못 매핑됐다",
  );
  assert(
  ...
  assert(
    payload.source_year === 2025 && payload.note === "메모",
    "메타 매핑이 틀렸다",
  );
});
...
check("cutScaleOutOfRange — 수시에 백분위(87.5)를 넣으면 거부한다", () => {
  ...
  assert(
    errorTypes(result).includes("cutScaleOutOfRange"),
    `cutScaleOutOfRange 여야 한다: ${JSON.stringify(errorTypes(result))}`,
  );
...
check(
  "jungsiLooksLikeGrade — 정시 컷 3.2 는 거부가 아니라 경고다(백분위 3.2 도 합법)",
  () => {
    const result = parseGoalUniversityCutRowsFromXlsx(
    ...
    assert(
      result.errors.length === 0,
      "CHECK(0~100)를 통과하는 값이라 거부하면 안 된다",
    );
    assert(
      warningTypes(result).includes("jungsiLooksLikeGrade"),
      "경고가 없다",
    );
...
check("정시 백분위 0 은 합법 값이라 살아남는다(clean() 오판 회귀 가드)", () => {
  ...
  assert(
    result.errors.length === 0,
    `0 이 거부됐다: ${JSON.stringify(result.errors)}`,
  );
  assert(
    result.rows[0].avg_cut === 0,
    `0 이 null 로 접혔다: ${result.rows[0].avg_cut}`,
  );
  assert(
    !warningTypes(result).includes("cutMissing"),
    "0 을 빈 값으로 오판했다",
  );
});
...
check("naesinCutTooHigh — 수시 8.5 등급은 경고, 9 초과는 거부", () => {
  ...
  assert(
    warningTypes(warnResult).includes("naesinCutTooHigh"),
    "8.5 등급 경고가 없다",
  );
  ...
  assert(
    errorTypes(failResult).includes("cutScaleOutOfRange"),
    "9.5 는 거부여야 한다",
  );
});
...
check("cutTypeChanged — id 가 있는 행의 컷 종류를 바꾸면 거부한다", () => {
  ...
  assert(
    errorTypes(result).includes("cutTypeChanged"),
    `cutTypeChanged 여야 한다: ${JSON.stringify(errorTypes(result))}`,
  );
  assert(result.rows.length === 0, "거부된 행이 payload 에 남았다");
});
...
check(
  "idNotFound — DB 에 없는 id 는 거부한다(캐시 없이 매번 조회하는 근거)",
  () => {
    const result = parseGoalUniversityCutRowsFromXlsx(
...
check(
  "missingRequiredFields — 학과명이 비면 거부한다(빈 학과명은 매칭 불가)",
  () => {
    const result = parseGoalUniversityCutRowsFromXlsx(
    ...
    assert(
      errorTypes(result).includes("missingRequiredFields"),
      "missingRequiredFields 여야 한다",
    );
...
check("invalidCutType — 3종 밖 라벨은 거부한다", () => {
  ...
  assert(
    errorTypes(result).includes("invalidCutType"),
    "invalidCutType 여야 한다",
  );
});
...
check("invalidNumber / invalidSourceYear", () => {
  ...
  assert(
    errorTypes(badYear).includes("invalidSourceYear"),
    "invalidSourceYear 여야 한다",
  );
  ...
  assert(
    ...
    "빈 기준 연도는 null 로 통과해야 한다",
  );
});

check(
  "duplicateNaturalKey — 파일 안에서 (컷 종류, 대학명, 학과명)이 겹치면 뒤 행을 거부한다",
  () => {
    const result = parseGoalUniversityCutRowsFromXlsx(
    ...
    assert(
      errorTypes(result).includes("duplicateNaturalKey"),
      "duplicateNaturalKey 여야 한다",
    );
    assert(
      result.rows.length === 1 && result.rows[0].avg_cut === 2.5,
      "첫 행만 살아남아야 한다",
    );
  },
);
...
check("truncatedColumn — 잘림 마커가 있으면 행 전체를 거부한다", () => {
  ...
  assert(
    errorTypes(result).includes("truncatedColumn"),
    "truncatedColumn 이어야 한다",
  );
});
...
check("sheetNotFound — 시트가 없으면 즉시 반환한다", () => {
  ...
  assert(
    errorTypes(result).includes("sheetNotFound"),
    "sheetNotFound 여야 한다",
  );
...
check(
  "unknownSource — 알 수 없는 출처는 경고를 남기고 manual 로 저장된다",
  () => {
    const result = parseGoalUniversityCutRowsFromXlsx(
    ...
    assert(
      result.errors.length === 0,
      `거부가 아니라 경고여야 한다: ${JSON.stringify(result.errors)}`,
    );
    assert(
      warningTypes(result).includes("unknownSource"),
      "unknownSource 경고가 있어야 한다",
    );
...
check("알려진 출처 라벨과 빈 출처는 unknownSource 경고를 내지 않는다", () => {
  ...
  assert(
    !warningTypes(known).includes("unknownSource"),
    "알려진 라벨은 경고 대상이 아니다",
  );
  assert(
  ...
  assert(
    !warningTypes(blank).includes("unknownSource"),
    "빈 출처는 경고 대상이 아니다",
  );
...
check("cutMissing / inactiveRow 경고", () => {
  ...
  assert(
    warningTypes(result).includes("inactiveRow"),
    "inactiveRow 경고가 없다",
  );
...
check("summary 집계가 rows/errors/warnings 와 일치한다", () => {
  ...
  assert(
    result.summary.willUpdate === 1,
    `willUpdate=1 이어야 한다: ${result.summary.willUpdate}`,
  );
  assert(
    result.summary.willInsert === 2,
    `willInsert=2 여야 한다: ${result.summary.willInsert}`,
  );
  assert(
    result.summary.willSkip === 1,
    `willSkip=1 이어야 한다: ${result.summary.willSkip}`,
  );
  assert(
    result.rows.length === 3,
    "payload 행수가 willInsert+willUpdate 와 다르다",
  );
  assert(
    result.summary.errorCounts.cutScaleOutOfRange === 1,
    "errorCounts 가 type 별로 안 세어졌다",
  );
  assert(
    result.summary.warningCounts.jungsiLooksLikeGrade === 1,
    "warningCounts 가 type 별로 안 세어졌다",
  );
});

check(
  "insert 배치와 update 배치의 키 집합이 각각 균일하다(PostgREST 키집합 해석 방어)",
  () => {
    const result = parseGoalUniversityCutRowsFromXlsx(
      makeWorkbook([
        row({ id: 7, avgCut: 3.3 }),
    ...
    assert(
      new Set(inserts.map(keysOf)).size === 1,
      "insert 배치의 키 집합이 균일하지 않다",
    );
    assert(
      new Set(updates.map(keysOf)).size === 1,
      "update 배치의 키 집합이 균일하지 않다",
    );
  },
);
