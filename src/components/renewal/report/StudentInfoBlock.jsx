...
export default function StudentInfoBlock({ student }) {
  return (
    ...
    <div className="fd-student-info flex w-full flex-col gap-1.5 lg:w-[24.375rem]">
      ...
      {/* 행 높이 고정(h-5) 금지 — q8 성적 흐름 옵션 6개 중 4개와 자유입력 희망 진로가 값 칸
          w-[12.5rem] 에서 2줄로 접힌다(실측). min-h-5 + items-start 로 행이 값 줄바꿈만큼
          자연스럽게 늘어나게 하고, 라벨은 위 라인에 정렬된 채로 남긴다. */}
      <div className="flex flex-col gap-1.5">
        {INFO_FIELDS.map(({ label, key }) => (
          <div key={key} className="fd-student-row flex min-h-5 items-start gap-4 lg:gap-[2.5rem]">
            ...
            <span className="fd-student-label w-20 shrink-0 text-base font-medium leading-[1.25rem] text-[#6b6b6b] lg:w-[5.6875rem] lg:text-[#d7d7d7]">
              {label}
            </span>
