...
export default function QuestionCard({ step, label, title, description, children }) {
  return (
    <div className="w-full rounded-[1.5rem] bg-white px-[3.75rem] py-[2.5rem]">
      ...
      <h2 className="mt-[1.5rem] text-[1.5rem] font-bold leading-[1.4] text-ink-strong">{title}</h2>

      {/* 설명문은 옵셔널 슬롯 — part-01 #2(카드 B)처럼 없는 스텝도 있다(part-01 구현 노트). */}
      {description && <p className="mt-[0.5rem] text-[0.875rem] leading-[1.5] text-ink-sub">{description}</p>}
