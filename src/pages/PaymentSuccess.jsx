...
function buildRows({ payment, orderId, totalAmount, vat }) {
  ...
  if (virtualAccount) {
    ...
    rows.push({
      label: '입금기한',
      value: deadline.remaining ? `${deadline.text} (${deadline.remaining})` : deadline.text
    });
  } else {
      ...
      <main className="min-h-screen bg-white pt-16">
        ...
        {status === 'error' && (
          <div className="flex min-h-[60vh] flex-col items-center justify-center px-6 text-center">
            ...
            {errorMsg && <p className="mt-4 text-xs leading-4 text-ink-sub">{errorMsg}</p>}
        ...
        {status === 'done' && (
          ...
          <div className="mx-auto w-full max-w-content px-5 py-12 text-center sm:px-8 sm:py-16">
            ...
            <h1 className="mt-8 text-2xl font-semibold leading-[1.9375rem] tracking-[-0.02em] text-ink sm:text-[3.125rem] sm:leading-[4.375rem]">
              {isWaitingDeposit ? '입금이 확인되면 이용이 시작돼요' : '주문이 완료됐어요!'}
            </h1>
