...
// `max-w`로 카드 폭(35rem, 2026-09-06 스케일 축소 — 기존 37.25rem)을 넘지 않는 폭에서
// 자르고 말줄임(`truncate`)한다.
...
export default function MetaTag({ children }: MetaTagProps) {
  return (
    <span
      ...
      className="inline-flex h-7.5 max-w-50 shrink-0 items-center truncate rounded-perf-modal bg-performance-tag px-1.5 text-app-label font-medium text-ink"
    >
      {children}
    </span>
  );
