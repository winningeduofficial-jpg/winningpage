...
const REPORT_COLUMNS = 'id,submission_id,report_type,sections,score,summary,model,prompt_version,created_at,updated_at';
...
function buildEvaluationSections(payload) {
  return EVALUATION_REPORT_SECTIONS.map((section) => {
    ...
    if (section.kind === 'note') {
      return { id: section.id, label: section.label, text: trimmed(payload.plagiarism) };
    }
...
function validateEvaluationPayload(payload) {
  ...
  for (const section of EVALUATION_REPORT_SECTIONS) {
    if (section.kind === 'triad') {
      const source = payload[section.id];
      if (!source || typeof source !== 'object') return { ok: false, reason: `${section.id}:missing` };
      for (const row of EVALUATION_TRIAD_ROW_LABELS) {
        if (!trimmed(source[row.key])) return { ok: false, reason: `${section.id}.${row.key}:empty` };
      }
      continue;
    }

    if (section.kind === 'note') {
      if (!trimmed(payload.plagiarism)) return { ok: false, reason: 'plagiarism:empty' };
      continue;
    }

    const source = payload.record_summary;
    if (!source || typeof source !== 'object') return { ok: false, reason: 'record_summary:missing' };
    for (const row of EVALUATION_RECORD_SUMMARY_ROW_LABELS) {
      if (!trimmed(source[row.key])) return { ok: false, reason: `record_summary.${row.key}:empty` };
    }
  }
 ...
 */
function buildReportEnvelope({ score, summary, sections, structure, submissionId, revision }) {
  return { v: 1, type: 'evaluation', score, summary, sections, structure, submissionId, revision };
}

/** 저장 봉투 → 응답 본문 공통부. 재생 경로와 신규 생성 경로가 같은 모양을 쓴다. */
function toClientReport(reportRow) {
  const envelope =
    reportRow?.sections && typeof reportRow.sections === 'object' ? reportRow.sections : {};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error('performance/evaluate 설정 오류:', error);
  ...
  try {
    ...
    if (existingError) throw new Error(`평가 리포트 조회 실패: ${existingError.message}`);

    // 같은 제출본 재요청 = 더블클릭·새로고침·응답 유실. **모델을 부르지 않는다.**
    if (existingReport && existingReport.submission_id === submissionRow.id && !regenerate) {
    ...
    if (designError) throw new Error(`설계 리포트 조회 실패: ${designError.message}`);
    if (!designReport) {
      return fail(res, 400, 'SESSION_INCOMPLETE', '설계 리포트를 먼저 받아 주세요.', {
        step: 4,
        charged: false
      });
    }
    ...
    const fields = submissionRow.fields && typeof submissionRow.fields === 'object' ? submissionRow.fields : {};
    const gate = checkSubmissionMinLength(schema, fields);

    // ── 게이트 ⑤ 제출물 게이트(§8.6 400 3종). 상한 게이트보다 **먼저** 본다 —
    //    형식 미달 요청이 재평가 슬롯을 태우면 안 된다.
    if (gate.total === 0) {
      return fail(res, 400, 'EMPTY_SUBMISSION', EMPTY_SUBMISSION_MESSAGE, { charged: false });
    }
    ...
    const designEnvelope =
      designReport.sections && typeof designReport.sections === 'object' ? designReport.sections : {};
    ...
    const userMsg = buildEvaluationUser({
      ...
      previousTopic: trimmed(sessionRow.previous_topic) || NO_PREVIOUS_TOPIC_TEXT,
      submissionText
    });
    ...
    try {
      for (let attempt = 0; attempt <= STRUCTURE_RETRY; attempt++) {
        ...
        try {
          response = await generateWithRetry({
            ...
            config: {
              ...
              temperature: isRetry ? 0.2 : EVALUATION_GENERATION_DEFAULTS.temperature,
          ...
          return fail(res, 502, 'MODEL_FAILED', '평가 리포트를 만들지 못했어요. 잠시 후 다시 시도해 주세요.', {
            charged: false
          });
        }
        ...
        if (finishReason === 'MAX_TOKENS') {
          ...
          console.warn(`performance/evaluate MAX_TOKENS 절단 (attempt ${attempt + 1})`);
          continue;
        }

        if (finishReason && finishReason !== 'STOP') {
          lastFailure = `finish-reason:${finishReason}`;
          console.warn(`performance/evaluate 비정상 종료 ${finishReason} (attempt ${attempt + 1})`);
          continue;
        }
        ...
        try {
          ...
          // ⓓ — 원문은 서버 로그에만 남긴다.
          console.error('performance/evaluate JSON 파싱 실패:', parseError?.message, rawText.slice(0, 400));
          continue;
        }
        ...
        if (!check.ok) {
          ...
          console.warn(`performance/evaluate 계약 위반 ${check.reason} (attempt ${attempt + 1})`);
          continue;
        }
    ...
    if (!payload) {
      ...
      return fail(res, 502, 'MODEL_FAILED', '평가 리포트를 정리하지 못했어요. 다시 시도해 주세요.', {
        charged: false
      });
    }
    ...
    if (commitError) {
      ...
      return fail(res, 500, 'INTERNAL', '평가 리포트 저장에 실패했습니다.', { charged: false });
    }
    ...
    return fail(res, 500, 'INTERNAL', '평가 리포트 생성에 실패했습니다.', { charged: false });
  }
}
