...
if (!fs.existsSync(SOURCE_HTML)) {
  ...
  if (start !== -1 && end !== -1) {
    ...
    for (const fixture of fixtures) {
      ...
      const external = externalBuildSubmissionText(
        fixture.topic,
        fixture.schema,
        fixture.fields,
      );
      const mine = ported.buildSubmissionText(
        fixture.topic,
        fixture.schema,
        fixture.fields,
      );
    ...
    check(
      ...
      ported
        .buildSubmissionText("주제", { fields: [] }, {})
        .includes("[제출 형식] 기본 보고서형"),
    );
  }
}
...
for (const [name, code] of [
  ...
  check(
    `${name} — consume_performance_credit 호출 없음`,
    !code.includes("consume_performance_credit"),
  );
  check(
    `${name} — program_access write 없음`,
    !/program_access[\s\S]{0,80}update/i.test(code),
  );
}
...
check(
  ...
  !SQL58_CODE.includes("performance_credit_ledger") &&
    !SQL58_CODE.includes("program_access"),
);
check(
  "3파일 모두 응답에 charged:false를 싣는다(§8.6 무차감 명시)",
  [SUBMISSION_API, EVALUATE_API, FINALIZE_API].filter((src) =>
    src.includes("charged: false"),
  ).length >= 2 &&
...
check(
  ...
  SUBMISSION_API.includes("body.mode === 'submit'") &&
    SUBMISSION_API.includes("body.mode === 'draft'"),
);
...
// ── Gemini 결함 완화 ⓐ~ⓓ (§8.4)
check(
  "evaluate.js — MAX_TOKENS면 파싱하지 않고 재시도(ⓒ)",
  EVALUATE_API.includes("finishReason === 'MAX_TOKENS'"),
);
check(
  "evaluate.js — responseSchema 강제",
  EVALUATE_API.includes("responseSchema: EVALUATION_REPORT_SCHEMA"),
);
check(
  "evaluate.js — score는 서버가 파싱(ⓐ)",
  EVALUATE_API.includes("parseEvaluationScore"),
);
check(
  "evaluate.js — maxDuration 60 선언",
  EVALUATE_API.includes("runtime: 'nodejs', maxDuration: 60"),
);
check(
...
for (const [name, source, code] of [
  ...
  check(
    `${name} — error:{code,message} 형태`,
    source.includes("error: { code, message }"),
  );
  check(
    `${name} — detail/error_message 키 없음`,
    !/\b(detail|error_message)\s*:/.test(code),
  );
  check(
    `${name} — catch에서 error.message를 응답에 싣지 않는다`,
    !/return\s+fail\([^)]*error\.message/.test(code) &&
      !/json\(\{[^}]*error\.message/.test(code),
  );
}
...
check(
  "performance_reports.submission_id 추가",
  SQL58.includes("add column if not exists submission_id uuid"),
);
check(
  "상한 2축 컬럼 추가",
...
check(
  "rag_use 승격이 정확히 2지점",
  ragPromotions.length === 2,
  `count=${ragPromotions.length}`,
);
check(
...
check(
  ...
  (SQL58.match(/revoke all on function/g) || []).length === 6 &&
    (
      SQL58.match(/grant execute on function[\s\S]{0,200}to service_role;/g) ||
      []
    ).length === 2,
);
check(
...
for (const column of ["evaluation_count", "evaluation_attempt_count"]) {
  check(
    `evaluate.js가 select 하는 ${column}이 sql/58에 있다`,
    EVALUATE_API.includes(column) && SQL58.includes(column),
  );
}
check(
...
if (failures.length) {
  console.error(
    `FAIL verify-performance-submission-api — ${failures.length}건\n`,
  );
...
console.log(
  "PASS verify-performance-submission-api — STEP5 API 3종 계약이 명세와 일치합니다.",
);
