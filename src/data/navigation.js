...
// 대신 두 그리드의 "피치"(셀/컬럼 폭 + gap)를 148px로 동일하게 맞춰(nav: 100+48,
// 메가: 140+8) 컬럼 i의 중심이 항상 nav 셀 i의 중심과 정렬되게 하고, MEGA_GUARD로
// 메가 그리드 시작점을 nav 그리드보다 20px((140-100)/2) 앞당겨 컬럼 0의 중심까지 맞춘다.
//
// NAV_CELL_W / NAV_CELL_GAP (0729 시안 실측 — nav 5항목):
//   각 항목 100px(6.25rem) 셀 중앙정렬, 셀 간 gap 48px(3rem) 고정(피치 148px).
//   과거 유동 clamp(gap)는 폐기 — 시안이 고정값이라 뷰포트별 보간이 불필요해졌다.
...
//   고정값 — nav와 임의로 다른 값을 쓰면 컬럼-셀 중심 정렬이 뷰포트에 따라 어긋난다.
//
...
// MEGA_GUARD: NAV_GUARD와 동일하게 대부분 0으로 평가되며, 컬럼-셀 중심 정렬을 위한
//   -1.25rem(20px) 상수 보정만 항상 적용된다(위 피치 설명 참고).
export const NAV_GUARD = 'max(0px, calc(6.04rem - (100vw - 72.75rem) / 2))';
export const MEGA_GUARD = `calc(${NAV_GUARD} - 1.25rem)`;
