...
import { columnSchema } from "@/components/editor/columnSchema";
 ...
 */
type ColumnBodyBlockNoteProps = {
  /** blocks는 non-empty 배열이 보장된다(호출부 책임). 빈 배열이면 BlockNoteEditor.create가 throw한다. */
  blocks: Array<Record<string, unknown>>;
};

export default function ColumnBodyBlockNote({
  blocks,
}: ColumnBodyBlockNoteProps) {
  // deps [] : uncontrolled. blocks가 바뀌면 호출부가 key로 remount한다.
  // TODO(ts): BlockNote initialContent는 PartialBlock<typeof columnSchema.blockSchema>[]를 기대하지만
  // 이 컴포넌트는 columnSchema에 국한하지 않는 원시 JSON 블록을 그대로 받아 넘긴다.
  const editor = useCreateBlockNote(
    {
      schema: columnSchema,
      dictionary: ko,
      // biome-ignore lint/suspicious/noExplicitAny: 위 TODO 참고
  ...
  useEffect(() => {
    ...
    const node = wrapperRef.current?.querySelector<HTMLElement>(".bn-editor");
    if (!node) return;
