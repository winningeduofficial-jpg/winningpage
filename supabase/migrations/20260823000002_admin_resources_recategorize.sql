...
--     - 구 「관리자 설정」 → 직원관리
--     - 라벨 변경: 멘토 성공전략→멘토스 소개, 대학별 모집요강→대입 모집 요강,
--       (그룹) 관리자 설정→직원관리
--     - ⚠️ 기획표의 「대입합격→수시정시합격」은 채택하지 않는다. 그 결정이 더
--       옛날 것이고, 정시 이용자가 거의 없어 수시·정시를 묶어 「대입합격」으로
--       가기로 뒤집혔다(사용자 확정 2026-08-23, QA 49행). 20260822000010 시드가
--       '수시정시합격'으로 넣어둔 것을 여기서 되돌린다.
--
...
insert into public.admin_resources (key, group_title, label, sort_order) values
  ...
  ('admissionSusiJungsi',             '입시정보 관리',  '대입합격',             230),
