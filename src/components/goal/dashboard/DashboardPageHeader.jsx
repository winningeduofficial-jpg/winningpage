...
function SparkleIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true" className="shrink-0">
      <path d="M12 2c.6 3.7 2.3 5.4 6 6-3.7.6-5.4 2.3-6 6-.6-3.7-2.3-5.4-6-6 3.7-.6 5.4-2.3 6-6z" fill="#3799FF" />
    </svg>
  );
}
...
export default function DashboardPageHeader({ adviceType = 'ai', dateLabel, headline, className = '' }) {
  ...
  return (
    <header className={`flex flex-col gap-[1.1875rem] ${className}`.trim()}>
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-[0.9375rem] font-medium leading-[1.4] text-ink-sub">{dateLabel}</span>
      </div>
      <div className="flex items-center gap-3">
        <SparkleIcon />
        <h1 className="text-[1.75rem] font-bold leading-[1.4] text-ink-strong">{headline}</h1>
      </div>
    </header>
  );
}
