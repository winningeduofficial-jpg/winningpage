...
async function loadAdmissionSectionView() {
  ...
  const tmpFile = path.join(
    REPO_ROOT,
    `.tmp-block-render-verify-${Date.now()}-${Math.random().toString(36).slice(2)}.mjs`,
  );
...
async function main() {
  ...
  const render = (doc, sectionKey = "selection_method", surface = "public") =>
    renderToStaticMarkup(
      React.createElement(AdmissionSectionView, { doc, sectionKey, surface }),
    );
...

  {
    ...
    record(
      "table:selection — 클래스 5종 + badge has/none + 빈값 리터럴 -",
      pass,
      out,
    );
  }

  {
    ...
    record(
      "table:change — 클래스 + 빈값 폴백(no/title 리터럴, content muted)",
      pass,
      out,
    );
  }
...

  {
    ...
    const pass =
      ...
      hasClass(out, "muted");
    record(
      "table:recruitExact — 2단 헤더(rowSpan/colSpan) + series-cell + muted",
      pass,
      out,
    );
  }
  ...
  for (const variant of ["exam", "minimum", "recordInfo", "score", "special"]) {
    ...
    record(
      `table:${variant} — GenericTable idx0·1 left + muted, compact 미적용`,
      pass,
      out,
    );
...

  {
    ...
    const out = render(doc);
    record(
      "block:keyValue — 렌더 성공(legacy 대응 마크업 없음, 최소 구현)",
      out.includes("지원자격") && out.includes("고등학교 졸업"),
      out,
    );
    // 하위 호환 잠금 — `href` 확장(수행평가 설계 리포트 §8.5)이 들어온 뒤에도 href 없는 행은
    // 앵커를 만들지 않는다. 대입 모집요강 생성 경로는 href를 만들지 않으므로 이 경로가 정상이다.
    record(
      "block:keyValue — href 없으면 <a> 없음(기존 경로 불변)",
      !out.includes("<a "),
      out,
    );
  }

  {
    ...
    record(
      "block:keyValue — href 확장이 새 창 앵커 + 안전 속성을 낸다",
      pass,
      out,
    );
  }
...

  {
    ...
    const out = render(doc);
    record(
      "block:preText — admission-raw-pre admission-safe-text-block",
      findTagWithExactClass(
        out,
        "pre",
...

  {
    ...
    const out = render(doc);
    record(
      "block:emptyBox — admission-empty-box",
      findTagWithExactClass(out, "div", "admission-empty-box") &&
        out.includes("대학별고사일 없음"),
      out,
    );
  }

  {
    ...
    const out = render(doc);
    record(
      "block:note — admission-result-note(내용 있을 때만)",
      findTagWithExactClass(out, "div", "admission-result-note") &&
        out.includes("실제 안내 문구"),
      out,
    );
  }
  {
    ...
    const out = render(doc);
    record(
      "block:note — 빈 텍스트면 렌더 생략",
      !out.includes("admission-result-note"),
      out,
    );
  }
  ...
  {
    const doc = baseDoc([
      { kind: "footnote", items: ["교직과정", "자율전공 과정"] },
    ]);
    const out = render(doc);
    record(
      "block:footnote — items를 공백 join해 div 하나로",
      findTagWithExactClass(out, "div", "admission-footnote") &&
        out.includes("교직과정 자율전공 과정"),
      out,
    );
  }

  {
    ...
    const out = render(doc);
    record(
      "block:heading — admission-subhead",
      findTagWithExactClass(out, "div", "admission-subhead") &&
        out.includes("국어 환산표"),
      out,
    );
  }

  {
    ...
    const pass =
      ...
      out.includes("내용");
    record(
      "block:group — section.admission-special-block + wrapModifier=special",
      pass,
      out,
    );
  }

  {
    ...
    record(
      "block:rawHtml — SafeHtml + admission-existing-html 래퍼",
      pass,
      out,
    );
  }
  ...
  // ── 섹션 제목 + 루트 wrap ────────────────────────────────────────────
  {
    ...
    record(
      "root — 섹션 제목(HWP_SECTION_TITLES) + admission-raw-section-wrap 형제 배치",
      pass,
      out,
    );
  }
  ...
  // 생략해야 한다 — 안 그러면 어드민 화면에서 제목이 두 번 찍힌다.
  {
    ...
    record(
      "root — 선두 rawHtml에 이미 heading 있으면 제목 중복 렌더 안 함",
      pass,
      `titleCount=${titleCount} ${out}`,
    );
  }
  {
    const doc = baseDoc([
    ...
    record(
      "root — 선두 rawHtml에 heading 없으면 제목 정상 렌더",
      pass,
      `titleCount=${titleCount} ${out}`,
    );
  }
