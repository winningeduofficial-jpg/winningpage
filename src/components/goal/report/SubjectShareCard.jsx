...
export default function SubjectShareCard({ title, empty, emptyMessage, rows, tip }) {
  return (
    <GoalCard tone="neutral" className="flex min-h-[27.5625rem] flex-col gap-5 px-6 py-6">
      ...
      {empty ? (
        ...
        <div className="flex flex-1 flex-col justify-center gap-4">
          {rows.map((row) => (
            <StatProgressRow key={row.label} label={row.label} value={row.value} unit="%" max={100} />
          ))}
        </div>
      )}
