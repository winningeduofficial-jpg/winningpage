...
test('sortByScoreAsc: 전 영역 동점이면 입력 배열 순서를 그대로 보존한다', () => {
  const scores = { GOAL: 50, PLAN: 50, EXEC: 50, TIME: 50, FEEDBACK: 50, STABILITY: 50 };
  assert.deepEqual(sortByScoreAsc(PAGE1_AREAS, scores), [
    'GOAL', 'PLAN', 'EXEC', 'TIME', 'FEEDBACK', 'STABILITY'
  ]);
});

test('sortByScoreAsc: 부분 동점 — 최저는 앞으로, 동점 묶음은 인덱스 순', () => {
  // EXEC(40)만 최저 → 맨 앞. 나머지 50 동점 5개는 PAGE1_AREAS 인덱스 순.
  const scores = { GOAL: 50, PLAN: 50, EXEC: 40, TIME: 50, FEEDBACK: 50, STABILITY: 50 };
  assert.deepEqual(sortByScoreAsc(PAGE1_AREAS, scores), [
    'EXEC', 'GOAL', 'PLAN', 'TIME', 'FEEDBACK', 'STABILITY'
  ]);
});
