...
//
// QA 행306 — narrative(GET /api/goal/advice sections의 "[내일 계획 제시]"/"[다음 계획
// 제시]" 본문)를 기존 규칙 기반 과목·시간 배분(plan) 위에 문장으로 덧붙인다. narrative가
// 아직 없으면(로딩 중/미생성) 문장 없이 칩만 그린다 — placeholder 문구를 만들지 않는다.
...
export default function TomorrowPlanCard({
  plan,
  narrative = null,
}: TomorrowPlanCardProps) {
  return (
    <GoalCard tone="neutral" className="flex h-full flex-col gap-4 px-8 py-7">
      ...
      {narrative && (
        <p className="text-app-body leading-normal text-ink">{narrative}</p>
      )}
      {/* 내일 목표 시간이 0/미설정이면(온보딩 직후 등) 위젯이 스스로 빈 상태로 분기한다. */}
      {plan.length === 0 ? (
        <p className="text-app-body leading-normal text-ink-sub">
          내일 계획 산출 준비 중입니다.
        </p>
      ) : (
