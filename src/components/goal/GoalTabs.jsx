...
export default function GoalTabs({ tabs, value, onChange, ariaLabel, gap = '2rem' }) {
  ...
  return (
    <div role="tablist" aria-label={ariaLabel} className="flex w-fit items-center" style={{ gap }}>
      {tabs.map((tab, index) => {
        ...
        return (
          <button
            ...
            className={`h-[1.9375rem] w-fit shrink-0 text-[0.9375rem] leading-[1.3] transition-colors ${
              selected ? 'font-bold text-ink-strong' : 'font-normal text-ink-sub'
            }`}
          >
            {tab.label}
          </button>
