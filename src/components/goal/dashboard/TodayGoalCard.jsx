...
/**
 * @param {object} data 부모 Dashboard가 채운 표시용 데이터(studyHours/quickAddOptions/
 *   upperGoalRate/lowerGoalRate) — Dashboard.jsx의 mapTodayGoal() 결과.
 * @param {() => void} [onSaved] 저장 성공 시 호출. Dashboard의 기존 데이터 훅을 다시 실행해
 *   게이지(upperGoalRate/lowerGoalRate)·확률을 최신화하는 역할은 이 카드가 아니라 상위
 *   리로드 콜백이 진다(단일 진실 공급원을 Dashboard 쪽에 유지).
 */
export default function TodayGoalCard({ data, onSaved }) {
  ...
  const handleSubmit = async () => {
    ...
    setError(messages[result.kind] || '저장에 실패했어요. 잠시 후 다시 시도해 주세요.');
  };
  ...
  return (
    <GoalCard tone="neutral" className="flex h-[25.5625rem] flex-col gap-6 px-[2rem] py-[1.875rem]">
      ...
      <div className="flex flex-wrap items-center gap-4 rounded-xl bg-surface-04 px-[1.875rem] py-[1.5rem]">
        ...
        {error && <p className="w-full text-[0.8125rem] leading-[1.4] text-red-600">{error}</p>}
      </div>
