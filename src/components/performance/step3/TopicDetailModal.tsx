...
// 근거). ESC/딤 클릭/포커스 트랩 같은 동작 로직은 이제 shadcn Dialog(Base UI) 내장
// 동작으로 대체됐다 — 예전에는 goal `AppModal`과 `useModalBehavior` 훅을 공유했지만,
// 두 모달 모두 각자 Base UI Dialog로 옮겨가면서 그 훅에 대한 의존은 여기서 제거됐다
// (훅 자체는 `useModalBehavior`를 여전히 쓰는 다른 호출부가 있어 파일은 남아 있다).
//
...
//
// **접근성**: `role="dialog"`/포커스 트랩/포털은 이제 Base UI Dialog가 담당한다 —
// 예전에는 이 컴포넌트가 앱 루트에 대한 참조가 없는 저수준 모달이라 형제 트리에
// `inert`를 직접 걸 수 없었지만, Base UI Portal + Backdrop 조합으로 그 제약이
// 해소됐다. 다만 `aria-modal`은 Base UI Popup이 자동 배선하지 않으므로 아래
// `DialogPrimitive.Popup`에 `aria-modal="true"`를 리터럴로 남겨 보강한다.
...
export default function TopicDetailModal({
  ...
  return (
    ...
    >
      <DialogPortal>
        {/* 딤 — `#00000066`(검정 40%, §5.11 실측 = `performance-dim` 토큰). */}
        <DialogOverlay className="bg-performance-dim" />
        ...
        <DialogPrimitive.Popup
          ...
          // 디자인 확인 시 조정될 수 있다.
          className="fixed top-1/2 left-1/2 z-100 flex h-203.75 max-h-[90vh] w-[calc(100%-2rem)] max-w-190.5 -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-performance-line bg-[#fbfbfa] shadow-[0_24px_60px_rgba(0,0,0,0.24)] outline-none"
        >
