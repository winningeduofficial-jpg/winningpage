...
//
// @blocknote/core의 Block 제네릭 타입은 스키마별로 달라 이 파일에서 재현하지 않는다 —
// 이 모듈은 BlockNote 블록의 공통 런타임 셰이프(content/children)만 duck-typing으로 읽으므로
// 인자는 unknown[]로 받고 내부에서만 any로 접근한다.

function textFromInlineArray(content: any): string {
  ...
  if (content && Array.isArray(content.rows)) {
    return content.rows
      .map((row: any) =>
        (row.cells || [])
          .map((cell: any) =>
            textFromInlineArray(Array.isArray(cell) ? cell : cell?.content),
          )
          .join(" "),
      )
      .join("\n");
...
function blockText(block: any): string {
  ...
  const childTexts = Array.isArray(block.children)
    ? block.children.map(blockText).filter(Boolean)
    : [];
...
export function blocksToPlainText(blocks: unknown[]): string {
  ...
  return blocks
    .map((block: any) => ({
      ...
      keepEmpty: Array.isArray(block?.content),
    }))
