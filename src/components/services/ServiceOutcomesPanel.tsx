...
//     자기평가의 lg:max-w-218.75 패널 폭 제한도 같은 이유로 폐기된다.
//
// (b) 페이지별 차이 흡수:
//     - items.length 만으로 sm 이상 열 수가 결정된다(4장: 심화탐구・자기평가・수행평가,
//       5장: 목표관리). 별도 prop 이 없다.
//
// 페이지마다 있던 md 고정 높이(md:h-[10.75rem] / md:h-42.5 + 셀 md:h-[8.75rem])는
...
export default function ServiceOutcomesPanel({
  ...
  return (
    ...
    <div
      className={`mt-8 grid grid-cols-2 gap-6 rounded-xl border border-line bg-surface-footer px-6 py-5.5 sm:mt-10 ${OUTCOME_COLS[items.length] ?? OUTCOME_COLS[4]} sm:gap-0 sm:divide-x sm:divide-line sm:px-4 lg:mt-perf-inset`}
    >
      {items.map((item) => (
        ...
        >
          ...
          />
          <p className="text-[1.125rem] font-semibold leading-[1.4] tracking-[-0.02em] text-ink">
            {item.label}
          </p>
        </div>
      ))}
