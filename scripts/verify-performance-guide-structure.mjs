...
const PORT_FILE = path.join(REPO_ROOT, 'api/_lib/performance/guide-structure.js');

if (!fs.existsSync(SOURCE_FILE)) {
  console.log(`SKIP verify-performance-guide-structure — 이식 원본 없음 (${SOURCE_DIR})`);
  process.exit(0);
}
...
if (sliceStart < 0 || sliceEnd < 0 || sliceEnd <= sliceStart) {
  console.error('FAIL: 원본에서 도메인 규칙 구간을 찾지 못했습니다 (find-resources.js 구조 변경?).');
  process.exit(1);
}
...
assert.deepStrictEqual(
  declaredFns,
  ['isRubricLikeQuestion', 'extractAnswerQuestions', 'inferAssessmentStructure'],
...
const ported = await import(path.join(REPO_ROOT, 'api/_lib/performance/guide-structure.js'));
...
const CASES = [
  ...
  },
  {
    ...
    text: Array.from({ length: 13 }, (_, i) => `질문 ${i + 1}: ${i + 1}번 문항은 무엇을 묻나요?`).join('\n')
  },
  {
    label: 'B9. 빈 문자열',
    text: ''
...
console.log('\n[우선순위 전수 대조]');
check(`키워드 조합 ${1 << TRIGGERS.length}가지 전건 일치`, comboMismatch === 0, comboFirst);
...
for (const text of RUBRIC_CORPUS) {
  ...
  if (expected !== actual) {
    rubricMismatch += 1;
    console.log(`  FAIL ${JSON.stringify(text)} — 원본 ${expected} / 이식 ${actual}`);
  }
}

check(`isRubricLikeQuestion 코퍼스 ${RUBRIC_CORPUS.length}건 전건 일치`, rubricMismatch === 0);

// 명세 §12.2 2행이 "호출부와 함께 옮겨야 의도 보존"이라고 못박은 성질. 값 자체를 고정한다.
check(
  "isRubricLikeQuestion('') === true (§12.2 2행, 빈 문항은 답변 문항에서 제외)",
  ported.isRubricLikeQuestion('') === true && original.isRubricLikeQuestion('') === true
);
...
const sourceRegexLiterals = [...originalSlice.matchAll(/\/(?:\\.|\[(?:\\.|[^\]\\])*\]|[^/\\\n])+\/[gimsuy]*/g)]
  .map((m) => m[0])
...
for (const token of vocabulary) {
  const structureSame = JSON.stringify(original.inferAssessmentStructure(token))
    === JSON.stringify(ported.inferAssessmentStructure(token));
  const rubricSame = original.isRubricLikeQuestion(token) === ported.isRubricLikeQuestion(token);
...
if (failures.length) {
  console.error(`FAIL verify-performance-guide-structure — ${failures.length}건\n`);
  failures.forEach((failure) => console.error(`  - ${failure}`));
  process.exit(1);
}

console.log('PASS verify-performance-guide-structure — 안내문 판정이 원본과 동일합니다.');
