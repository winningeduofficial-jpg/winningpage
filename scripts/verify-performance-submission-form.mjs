...
const CHARS_FILE = path.join(REPO_ROOT, 'api/_lib/performance/submission-chars.js');
const SCHEMA_FILE = path.join(REPO_ROOT, 'api/_lib/performance/submission-schema.js');
const CLIENT_LIB = path.join(REPO_ROOT, 'src/lib/performance/submission.js');
const FORM_FILE = path.join(REPO_ROOT, 'src/components/performance/step5/SubmissionForm.jsx');
const COUNTER_FILE = path.join(REPO_ROOT, 'src/components/performance/step5/CharCounter.jsx');
...
async function loadHarness() {
  ...
  const harnessPath = path.join(REPO_ROOT, `.tmp-perf-submission-harness-${stamp}.jsx`);
  const bundlePath = path.join(REPO_ROOT, `.tmp-perf-submission-bundle-${stamp}.mjs`);
...
const render = (props) => renderToStaticMarkup(React.createElement(Harness, props));
...
const collect = (html, tagName) =>
  [...html.matchAll(new RegExp(`<${tagName}\\b([^>]*)>`, 'g'))].map((m) => attrsOf(m[0]));
const labelsOf = (html) =>
  [...html.matchAll(/<label\b([^>]*)>([\s\S]*?)<\/label>/g)].map((m) => ({
    ...attrsOf(m[0].slice(0, m[0].indexOf('>') + 1)),
    text: m[2].replace(/<[^>]*>/g, '').trim()
  }));
const idsInHtml = (html) => new Set([...html.matchAll(/\sid="([^"]+)"/g)].map((m) => m[1]));
...
const GUIDES = [
  ...
  ['question_based', '질문 1: 주제를 고른 이유는 무엇인가요?\n질문 2: 탐구 과정을 소개해 주세요\n질문 3: 배운 점은 무엇인가요?'],
  ['length_based_report', '분량은 1000자 내외로 작성하고 탐구 내용을 포함하세요.'],
...
for (const [expectedType, guide] of GUIDES) {
  const schema = schemaModule.normalizeSubmissionSchema(schemaModule.inferSubmissionSchema(guide));
...
for (const [type, schema] of schemas) {
  ...
  check(
    ...
    JSON.stringify(renderedNames) === JSON.stringify(schema.fields.map((f) => f.key)),
  ...
  const helperMissing = schema.fields.filter((field) => {
    ...
    return !described.some((id) => id.endsWith('-helper')) || !html.includes(field.helper);
  });
  check(
  ...
  const counterMissing = textareas.filter(
    (t) => !(t['aria-describedby'] || '').split(' ').some((id) => id.endsWith('-counter'))
  );
  check(
...
const manyQuestions = Array.from({ length: 25 }, (_, i) => `질문 ${i + 1}: ${i + 1}번 문항은 무엇인가요?`).join('\n');
const bigSchema = schemaModule.normalizeSubmissionSchema(schemaModule.inferSubmissionSchema(manyQuestions));
check('25문항 안내문 → 20필드로 절단(서버)', bigSchema.fields.length === 20, `${bigSchema.fields.length}개`);

const bigHtml = render({ schema: bigSchema, value: {}, topicTitle: '주제' });
check('20필드가 전부 렌더된다', collect(bigHtml, 'textarea').length === 20);
check('20필드의 id가 전부 유일하다', idsInHtml(bigHtml).size === [...bigHtml.matchAll(/\sid="/g)].length);
// 고정 높이 + 내부 스크롤(§5.14 제안). 자동 확장으로 바뀌면 20필드에서 채팅 스크롤이 튄다.
check(
  '텍스트영역은 고정 높이 h-40(10rem) + resize 잠금',
  collect(bigHtml, 'textarea').every((t) => /\bh-40\b/.test(t.class) && /\bresize-none\b/.test(t.class))
);
...
for (const [index, values] of COUNT_CASES.entries()) {
  ...
  const counters = [...html.matchAll(/<p id="[^"]*-counter"[^>]*>(\d+)자<\/p>/g)].map((m) => Number(m[1]));
  const expected = basic.fields.map((f) => charsModule.countFieldChars(values[f.key]));
  check(
...
for (const testCase of GATE_CASES) {
  const html = render({ schema: basic, value: testCase.values, topicTitle: '주제' });
...
for (const button of [submitButton(lockedHtml), saveButton(lockedHtml)]) {
  const described = (button?.['aria-describedby'] || '').split(' ').filter(Boolean);
  check(
...
const liveRegions = [...typingHtml.matchAll(/<[^>]*aria-live="polite"[^>]*>([\s\S]*?)</g)];
check(
...
check(
  ...
  /from '\.\.\/\.\.\/\.\.\/api\/_lib\/performance\/submission-chars\.js'/.test(clientText)
);
check(
...
for (const [name, text] of [
  ...
  check(
    ...
    !/replace\(\/\\s\+\/g/.test(text) && !/\[\.\.\.[A-Za-z]+\]\.length/.test(text)
  );
}
...
check(
  ...
  /SUBMISSION_MIN_CHARS/.test(formText) && !/(?:const|let|var)\s+\w+\s*=\s*100\b/.test(formText)
);
