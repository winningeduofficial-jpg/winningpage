...
// 바인딩으로 안전하다.
export function renderBlock(block: Block | null | undefined, key: Key) {
  ...
  switch (block.kind) {
    ...
    // `ordered`는 §8.5 블록 뷰 확장분(번호 목록). PlainListBlock JSDoc 스키마(admissionDoc.js,
    // 수정 범위 밖)에는 아직 없는 필드라 캐스트로만 읽는다 — 없으면 `undefined`가 넘어가
