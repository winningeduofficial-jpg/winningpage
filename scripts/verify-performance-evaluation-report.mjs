...
async function loadHarness() {
  ...
  const harnessPath = path.join(
    REPO_ROOT,
    `.tmp-perf-eval-harness-${stamp}.jsx`,
  );
...
const renderBranch = (props) =>
  renderToStaticMarkup(React.createElement(Branch, props));
...
check(
  headings.slice(1).join("|") ===
    EVALUATION_REPORT_SECTIONS.map((s) => s.label).join("|"),
...
for (const row of EVALUATION_TRIAD_ROW_LABELS) {
  ...
  check(
    count === 5,
    `3분할 행 '${row.label}'이 5개 섹션에 나와야 하는데 ${count}개다`,
  );
}
for (const row of EVALUATION_RECORD_SUMMARY_ROW_LABELS) {
  check(
    baseHtml.includes(`<b>${row.label}</b>`),
    `누적 기록용 요약 행 '${row.label}'이 없다`,
  );
}
...
// ─────────────────────────────────────────────────────────────────────
check(
  baseHtml.includes("/100"),
  "점수 카드에 만점 표기(`/100`)가 없다(§5.16 `86/100`)",
);
...
const noScoreHtml = renderModal({
  open: true,
  report: buildReport({ score: null }),
});
check(
  !noScoreHtml.includes("/100"),
  "score가 없을 때 만점 표기가 남았다 — 없는 점수를 지어내면 안 된다",
);
check(
...
check(
  !emptyHtml.includes("빈 섹션"),
  "본문이 빈 섹션이 라벨만 남은 채 렌더됐다",
);

// 닫힌 상태 / 리포트 없음 → 아무것도 렌더하지 않는다(무음 실패 방지 계약).
check(
  renderModal({ open: false, report: buildReport() }) === "",
  "open=false인데 렌더됐다",
);
check(
  renderModal({ open: true, report: null }) === "",
  "report=null인데 모달이 렌더됐다",
);
...
const limitedHtml = renderBranch({
...
const limitedNoteId = limitedButtons[0]?.attrs.match(
  /aria-describedby="([^"]+)"/,
);
check(
  Boolean(limitedNoteId) &&
    new RegExp(`id="${limitedNoteId?.[1]}"[^>]*>상한 안내`).test(limitedHtml),
...
const libSource = fs.readFileSync(
  path.join(REPO_ROOT, "src/lib/performance/evaluation.js"),
  "utf8",
);
...
check(
  pageSource.includes("<EvaluationReportModal") &&
    pageSource.includes("<EvaluationBranchActions"),
...
console.log(
  "✅ 평가 리포트 모달 + 분기 3버튼 검증 통과 (카드 1 + sections 7, §5.16/§5.17 계약)",
);
