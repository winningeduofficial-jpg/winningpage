 ...
 * @returns {{periodKey:string, start:string, end:string, periodLabel:string}}
 */
export function resolveWeeklyPeriod(periodParam, nowYmd) {
 ...
 * @returns {{periodKey:string, start:string, end:string, periodLabel:string}}
 */
export function resolveMonthlyPeriod(periodParam, nowYmd) {
...
export function computeStudyHours(records) {
  return round1(records.reduce((sum, r) => sum + toNum(r.study_hours), 0));
}
 ...
 * 코호트가 2명 미만이면 표본 부족.
 * @param {string} selfProfileId
 * @param {Array<{profileId:string, hours:number}>} cohortHours 코호트 전원(본인 포함)의 기간 합산 시간
 * @returns {{insufficientSample:boolean, rank:number|null, total:number, topPercent:number|null}}
 */
...
export function computeConditionBreakdown(records) {
  ...
  for (const record of records) {
    ...
    bucket.count += 1;
    bucket.sum += toNum(record.study_hours);
  }
...
export function computeDistraction(records) {
  const counts = {};
  for (const record of records) {
...
/** 완료한 핵심 학습 항목 카운트. */
export function computeCoreItems(records) {
  const counts = {};
  for (const record of records) {
...
export function buildMonthlyWeeks({
  ...
  while (cursor <= monthEnd) {
    ...
    const clippedStart = maxYmd(weekMonday, monthStart);
    const clippedEnd = minYmd(weekSunday, monthEnd);
 ...
 * @param {{university:string}} target
 * @param {{startValue:number|null, endValue:number|null}} snapshot susi/jeongsi 각각
 */
function buildAdmissionBlock(university, susiSnapshot, jeongsiSnapshot) {
  function toDelta(snapshot) {
    const start = toNum(snapshot.startValue, 0);
...
/**
 * @param {{ideal:{university,department}, min:{university,department}}} targets
 * @param {object|null} startLog goal_probability_logs 스냅샷(기간 시작 이전 최신) 또는 null
 * @param {object|null} endLog goal_probability_logs 스냅샷(기간 끝까지 최신) 또는 null
 */
export function computeAdmissionDelta({ targets, startLog, endLog }) {
