...
const RICH_BASE = 'bn-doc max-w-[45rem] break-keep text-base font-normal leading-8 text-[#525252]';
...
class ColumnBodyBoundary extends Component {
  ...
  componentDidCatch(error) {
    if (import.meta.env?.DEV) console.error('[ColumnBody] block 렌더 실패, 평문으로 대체', error);
  }
...
export default function ColumnBody({ post, className = '' }) {
  ...
  const plain = <div className={`${PLAIN_BASE} ${className}`}>{post?.content}</div>;
